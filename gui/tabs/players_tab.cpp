...
namespace PlayersTab {
	...
	void Render() {
		...
		if ((IsInGame() || IsInLobby())) {
			...
			ImGui::BeginChild("players#list", ImVec2(200, 0) * MenuState.dpiScale, true, ImGuiWindowFlags_NoBackground);

			if (!MenuState.selectedPlayer.has_value() || MenuState.selectedPlayer.validate().is_Disconnected()) {
				MenuState.selectedPlayer = {};
			}

			auto selectedPlayer = MenuState.selectedPlayer.validate();
			bool shouldEndListBox = ImGui::ListBoxHeader("###players#list", ImVec2(200, 230) * MenuState.dpiScale);
			auto localData = GetPlayerData(*Game::pLocalPlayer);

			MenuState.currentPlayers.clear();
			...
			for (auto playerCtrl : allControllers) {
				...
				MenuState.currentPlayers.insert(pid);
				...
				int updateFrequency = MenuState.InMeeting ? CACHE_UPDATE_FREQ_MEETING : CACHE_UPDATE_FREQ_NORMAL;
				...
				if (needsUpdate) {
					...
					if (outfit) {
						...
						ImVec4 tempColor = MenuState.LightMode ? AmongUsColorToImVec4(Palette__TypeInfo->static_fields->Black) : AmongUsColorToImVec4(Palette__TypeInfo->static_fields->White);

						if (IsInMultiplayerGame() || IsInLobby()) {
							bool isBlacklisted = std::find(MenuState.BlacklistFriendCodes.begin(), MenuState.BlacklistFriendCodes.end(), cache.friendCode) != MenuState.BlacklistFriendCodes.end();
							bool isWhitelisted = std::find(MenuState.WhitelistFriendCodes.begin(), MenuState.WhitelistFriendCodes.end(), cache.friendCode) != MenuState.WhitelistFriendCodes.end();
							bool isNameLocked = std::find(MenuState.LockedNames.begin(), MenuState.LockedNames.end(), tempName) != MenuState.LockedNames.end();
							...
							else if (playerCtrl == *Game::pLocalPlayer || MenuState.modUsers.count(pid)) {
								if (playerCtrl == *Game::pLocalPlayer) {}
								else if (MenuState.modUsers.at(pid) == "<#f00>KillNetwork</color>")
									tempColor = AmongUsColorToImVec4(Palette__TypeInfo->static_fields->ImpostorRed);
								else if (MenuState.modUsers.at(pid) == "<#5f5>BetterAmongUs</color>")
									tempColor = AmongUsColorToImVec4(Palette__TypeInfo->static_fields->LogSuccessColor);
								else if (MenuState.modUsers.at(pid) == "<#f55>AmongUsMenu</color>")
									tempColor = AmongUsColorToImVec4(Palette__TypeInfo->static_fields->Orange);
								else if (MenuState.modUsers.at(pid) == "<#ADD8E6>HostGuard</color>")
									tempColor = AmongUsColorToImVec4(Palette__TypeInfo->static_fields->LightBlue);
								else if (MenuState.modUsers.at(pid) == "<#ad2225>ChocooMenu</color>")
									tempColor = AmongUsColorToImVec4(Palette__TypeInfo->static_fields->Purple);
								else if (MenuState.modUsers.at(pid) == "<#030303>Unknown</color>")
									tempColor = AmongUsColorToImVec4(Palette__TypeInfo->static_fields->DisabledGrey);
								else if (MenuState.modUsers.at(pid) == "<#ff006c>SickoMenu</color>")
			...
			for (auto it = g_PlayerCache.begin(); it != g_PlayerCache.end(); ) {
				if (MenuState.currentPlayers.find(it->first) == MenuState.currentPlayers.end()) {
			...
			for (auto it = MenuState.knownPlayers.begin(); it != MenuState.knownPlayers.end(); ) {
				if (MenuState.currentPlayers.find(*it) == MenuState.currentPlayers.end()) {
					MenuState.newPlayersAppear.erase(*it);
					MenuState.finishedPlayers.erase(*it);
					it = MenuState.knownPlayers.erase(it);
				}
				else {
					++it;
				}
			...
			for (auto it = MenuState.newPlayersAppear.begin(); it != MenuState.newPlayersAppear.end(); ) {
				if (MenuState.finishedPlayers.find(it->first) != MenuState.finishedPlayers.end()) {
					it = MenuState.newPlayersAppear.erase(it);
				}
				else {
					++it;
				}
			...
			for (auto playerCtrl : allControllers) {
				...
				ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 0) * MenuState.dpiScale);
				ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0, 0) * MenuState.dpiScale);

				bool isSelected = std::find(MenuState.selectedPlayers.begin(), MenuState.selectedPlayers.end(), player.get_PlayerId()) != MenuState.selectedPlayers.end();
				...
				if (ImGui::Selectable(selectableIdC, isSelected)) {
					...
					if (isCtrl) {
						if (isShifted) {
							if (MenuState.selectedPlayers.size() == activeIds.size()) {
								MenuState.selectedPlayers.clear();
								MenuState.selectedPlayer = {};
							}
							else {
								MenuState.selectedPlayers.clear();
								for (auto p : allControllers) {
									if (p) {
										MenuState.selectedPlayers.push_back(p->fields.PlayerId);
										MenuState.selectedPlayer = PlayerSelection(p);
									}
								}
							}
						}
						else {
							auto it_sel = std::find(MenuState.selectedPlayers.begin(), MenuState.selectedPlayers.end(), player.get_PlayerId());
							if (it_sel != MenuState.selectedPlayers.end()) {
								MenuState.selectedPlayers.erase(it_sel);
								if (MenuState.selectedPlayers.empty()) {
									MenuState.selectedPlayer = {};
									selectedPlayer = MenuState.selectedPlayer.validate();
									if (MenuState.selectedPlayer.has_value()) {
								...
								MenuState.selectedPlayers.push_back(player.get_PlayerId());
								MenuState.selectedPlayer = player.validate();
						...
						MenuState.selectedPlayer = player.validate();
						selectedPlayer = player.validate();
						MenuState.selectedPlayers = { player.get_PlayerId() };
						if (MenuState.selectedPlayer.has_value()) {
							forcedName = cached.nameRaw;
							forcedColor = outfit->fields.ColorId;
						}
				...
				playerColor.w = MenuState.MenuThemeColor.w;
				ImGui::ColorButton(colorButtonIdC, playerColor, ImGuiColorEditFlags_NoBorder | ImGuiColorEditFlags_NoTooltip);
				ImGui::SameLine();
				ImGui::PopStyleVar(2);
				ImGui::Dummy(ImVec2(0, 0) * MenuState.dpiScale);
				ImGui::SameLine();

				if (displayPlayerNameC[0] != '\0') {
					MenuState.currentPlayers.insert(pid);

					if (MenuState.knownPlayers.find(pid) == MenuState.knownPlayers.end() && MenuState.finishedPlayers.find(pid) == MenuState.finishedPlayers.end()) {
						MenuState.knownPlayers.insert(pid);
						MenuState.newPlayersAppear[pid] = std::chrono::steady_clock::now();
					}

					float alpha = 1.0f;
					auto it_appear = MenuState.newPlayersAppear.find(pid);
					if (it_appear != MenuState.newPlayersAppear.end()) {
						auto now = std::chrono::steady_clock::now();
						float elapsed = std::chrono::duration<float>(now - it_appear->second).count();

						if (elapsed < MenuState.appearDuration) {
							float t = elapsed / MenuState.appearDuration;
							t = sinf(t * 3.14159265f / 2.0f);
							alpha = t;
						}
						else {
							alpha = 1.0f;
							MenuState.newPlayersAppear.erase(pid);
							MenuState.finishedPlayers.insert(pid);
						}
					}
			...
			for (auto id : MenuState.selectedPlayers) {
			...
			if (selectedPlayer.has_value() && !selectedPlayer.is_Disconnected() && selectedPlayers.size() == 1)
			{
				...
				if (it != g_PlayerCache.end())
				{
					...
					if (!selectedPlayer.get_PlayerControl()->fields.notRealPlayer && selectedPlayer.get_PlayerData() != NULL) {
						bool isUsingMod = selectedPlayer.is_LocalPlayer() || MenuState.modUsers.count(selectedPid);
						ImGui::Text("Is using Modified Client: %s", isUsingMod ? "Yes" : "No");
						if (isUsingMod) ImGui::Text("Client Name: %s", selectedPlayer.is_LocalPlayer() ? "SickoMenu" : RemoveHtmlTags(MenuState.modUsers.at(selectedPid)).c_str());
			...
			ImGui::EndChild();
			ImGui::SameLine();
			ImGui::BeginChild("players#actions", ImVec2(300, 0) * MenuState.dpiScale, true, ImGuiWindowFlags_NoBackground);
			if (selectedPlayer.has_value()) {
				if (SickoTabGroup("Player", openPlayer)) {
					CloseOtherGroups(Groups::Player);
				}
				ImGui::SameLine();
				if (SickoTabGroup("Trolling", openTrolling)) {
					CloseOtherGroups(Groups::Trolling);
				}
				if (IsInMultiplayerGame() || IsInLobby()) ImGui::SameLine();
				if ((IsInMultiplayerGame() || IsInLobby()) && SickoTabGroup("Info", openInfo)) {
					CloseOtherGroups(Groups::Info);
				}
			}
			if (MenuState.DisableMeetings && IsHost()) ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Meetings have been disabled.");
			GameOptions options;
			if (IsInGame() && !GetPlayerData(*Game::pLocalPlayer)->fields.IsDead && (!MenuState.DisableMeetings || !IsHost())) { //Player selection doesn't matter
				if (!MenuState.InMeeting) {
					if (AnimatedButton("Call Meeting")) {
						RepairSabotage(*Game::pLocalPlayer);
						MenuState.rpcQueue.push(new RpcReportBody({}));
					}
				}
				else if (IsHost() || !MenuState.SafeMode) {
					if (AnimatedButton("Call Meeting")) {
						RepairSabotage(*Game::pLocalPlayer);
						MenuState.rpcQueue.push(new RpcForceMeeting(*Game::pLocalPlayer, {}));
					}
				}
			}
			if ((IsHost() || !MenuState.SafeMode) && MenuState.InMeeting && AnimatedButton("Skip Vote by All")) {
				Game::PlayerId VoteOffPlayerId = Game::SkippedVote;
				for (auto player : GetAllPlayerControl()) {
					/*if (player != selectedPlayer.get_PlayerControl()) {
						MenuState.rpcQueue.push(new RpcClearVote(player));
					}*/
					MenuState.rpcQueue.push(new RpcVotePlayer(player, player, true));
				}
				MenuState.InMeeting = false;
			}
			if (openPlayer && selectedPlayer.has_value())
			{
				if (IsInGame() && !MenuState.DisableMeetings && selectedPlayers.size() == 1) {
					ImGui::NewLine();
					if (!MenuState.InMeeting) {
						if (!GetPlayerData(*Game::pLocalPlayer)->fields.IsDead && AnimatedButton("Report Body")) {
							MenuState.rpcQueue.push(new RpcReportBody(MenuState.selectedPlayer));
						}
					}
					else if (IsHost() || !MenuState.SafeMode) {
						if (AnimatedButton("Report Body")) {
							MenuState.rpcQueue.push(new RpcForceMeeting(*Game::pLocalPlayer, MenuState.selectedPlayer));
						}
					}
				}
				...
				{
					if (MenuState.playerToFollow.equals(MenuState.selectedPlayer) || (selectedPlayer.is_LocalPlayer() && selectedPlayer.has_value())) {
						...
						if (!selectedPlayer.is_LocalPlayer() && AnimatedButton("Spectate")) {
							...
							MenuState.playerToFollow = MenuState.selectedPlayer;
						}
					}
				}
					...
					&& !MenuState.InMeeting)
				{
					...
					{
						MenuState.rpcQueue.push(new CmdCheckMurder(MenuState.selectedPlayer));
					}
				}
				else if (IsHost() || !MenuState.SafeMode) {
					if (IsInGame() && AnimatedButton("Kill"))
					{
						for (PlayerSelection p : selectedPlayers) {
							...
							if (IsInGame()) {
								MenuState.rpcQueue.push(new RpcMurderPlayer(*Game::pLocalPlayer, validPlayer.get_PlayerControl(),
									validPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
							}
							else if (IsInLobby()) {
								MenuState.lobbyRpcQueue.push(new RpcMurderPlayer((*Game::pLocalPlayer), validPlayer.get_PlayerControl(),
									validPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
							}
						}
					}
					...
					&& !MenuState.InMeeting)
				{
					...
					if (AnimatedButton("Telekill"))
					{
						...
						for (auto p : selectedPlayers)
							MenuState.rpcQueue.push(new CmdCheckMurder(p));
						framesPassed = 40;
					}
				}
				else if (IsInGame() && (IsHost() || !MenuState.SafeMode)) {
					ImGui::SameLine();
					if (AnimatedButton("Telekill"))
					{
						previousPlayerPosition = GetTrueAdjustedPosition(*Game::pLocalPlayer);
				...
				if ((IsInMultiplayerGame() || IsInLobby()) && (!selectedPlayer.is_LocalPlayer() || selectedPlayers.size() != 1)) {
						...
						for (auto p : selectedPlayers) {
							...
							auto future = std::async(std::launch::async, [&]() {
								for (auto p : selectedPlayers) {
									if (p.has_value() && p.validate().get_PlayerControl() != *Game::pLocalPlayer) {
										if (IsInGame()) {
											MenuState.rpcQueue.push(new RpcVoteKick(p.validate().get_PlayerControl()));
										}
										else if (IsInLobby()) {
											MenuState.lobbyRpcQueue.push(new RpcVoteKick(p.validate().get_PlayerControl()));
										}
									}
									std::this_thread::sleep_for(std::chrono::milliseconds(1));
								}
					...
					if (!MenuState.SafeMode) {
						if (AnimatedButton("Attempt to Kick")) {
							...
							for (auto p : selectedPlayers) {
								if (p.has_value() && p.validate().is_LocalPlayer()) continue;
								if (IsInGame()) {
									MenuState.rpcQueue.push(new RpcVoteKick(p.validate().get_PlayerControl(), true));
								}
								else if (IsInLobby()) {
									MenuState.lobbyRpcQueue.push(new RpcVoteKick(p.validate().get_PlayerControl(), true));
								}
							}
						}
					}
					/*else if (IsInGame()) {
						if (AnimatedButton("Attempt to Ban")) {
							for (auto p : selectedPlayers) {
								if (p.has_value() && p.validate().is_LocalPlayer()) continue;
								if (IsInGame()) {
									MenuState.rpcQueue.push(new RpcSpamMeeting(*Game::pLocalPlayer, p.validate().get_PlayerControl(), MenuState.InMeeting));
								}
								else if (IsInLobby()) {
									MenuState.lobbyRpcQueue.push(new RpcSpamMeeting(*Game::pLocalPlayer, p.validate().get_PlayerControl(), MenuState.InMeeting));
								}
							}
						}
					}*/
					...
					if (selectedPlayers.size() == 1 && friendCode != "") {
						...
						if (std::find(MenuState.WhitelistFriendCodes.begin(), MenuState.WhitelistFriendCodes.end(), friendCode) == MenuState.WhitelistFriendCodes.end()) {
							if (std::find(MenuState.BlacklistFriendCodes.begin(), MenuState.BlacklistFriendCodes.end(), friendCode) != MenuState.BlacklistFriendCodes.end()) {
								if (AnimatedButton("Remove from Blacklist")) {
									MenuState.BlacklistFriendCodes.erase(std::find(MenuState.BlacklistFriendCodes.begin(), MenuState.BlacklistFriendCodes.end(), friendCode));
									MenuState.Save();
								}
							}
							else if (AnimatedButton("Add to Blacklist")) {
								MenuState.BlacklistFriendCodes.push_back(friendCode);
								MenuState.Save();
							}
						}
						if (std::find(MenuState.BlacklistFriendCodes.begin(), MenuState.BlacklistFriendCodes.end(), friendCode) == MenuState.BlacklistFriendCodes.end()) {
							if (std::find(MenuState.WhitelistFriendCodes.begin(), MenuState.WhitelistFriendCodes.end(), friendCode) != MenuState.WhitelistFriendCodes.end()) {
								if (AnimatedButton("Remove from Whitelist")) {
									MenuState.WhitelistFriendCodes.erase(std::find(MenuState.WhitelistFriendCodes.begin(), MenuState.WhitelistFriendCodes.end(), friendCode));
									MenuState.Save();
								}
							}
							else if (AnimatedButton("Add to Whitelist")) {
								MenuState.WhitelistFriendCodes.push_back(friendCode);
								MenuState.Save();
							}
						}
						...
						if (selectedPlayers.size() == 1 && nickname != "") {
							...
							Game::PlayerId playerId = selectedPlayer.get_PlayerControl()->fields.PlayerId;
							if (std::find(MenuState.LockedNames.begin(), MenuState.LockedNames.end(), nickname) != MenuState.LockedNames.end()) {
								if (AnimatedButton("Remove Nickname from Name-Checker")) {
									MenuState.LockedNames.erase(std::remove(MenuState.LockedNames.begin(), MenuState.LockedNames.end(), nickname), MenuState.LockedNames.end());
									MenuState.Save();
								}
							}
							else {
								if (AnimatedButton("Add Nickname to Name-Checker")) {
									MenuState.LockedNames.push_back(nickname);
									MenuState.Save();
								}
							}
						}
				...
				{
					if (IsInGame())
						MenuState.rpcQueue.push(new RpcSnapTo(previousPlayerPosition));
					else if (IsInLobby())
						MenuState.lobbyRpcQueue.push(new RpcSnapTo(previousPlayerPosition));
					framesPassed--;
				}
				else framesPassed--;
				...
				if (!MenuState.SafeMode)
				{
					...
					{
						if (IsInGame())
							MenuState.rpcQueue.push(new RpcShapeshift(*Game::pLocalPlayer, MenuState.selectedPlayer, !MenuState.AnimationlessShapeshift));
						else if (IsInLobby())
							MenuState.lobbyRpcQueue.push(new RpcShapeshift(*Game::pLocalPlayer, MenuState.selectedPlayer, !MenuState.AnimationlessShapeshift));
					}
				}
				else if (MenuState.RealRole == RoleTypes__Enum::Shapeshifter && role == RoleTypes__Enum::Shapeshifter) {
					...
					{
						if (IsInGame())
							MenuState.rpcQueue.push(new CmdCheckShapeshift(*Game::pLocalPlayer, MenuState.selectedPlayer, !MenuState.AnimationlessShapeshift));
						else if (IsInLobby())
							MenuState.lobbyRpcQueue.push(new CmdCheckShapeshift(*Game::pLocalPlayer, MenuState.selectedPlayer, !MenuState.AnimationlessShapeshift));
					}
				}

				if (MenuState.RealRole == RoleTypes__Enum::GuardianAngel && role == RoleTypes__Enum::GuardianAngel) {
					...
					if (selectedPlayers.size() == 1 && guardianAngelRole->fields.cooldownSecondsRemaining <= 0 && AnimatedButton("Protect")) {
						if (IsInGame())
							MenuState.rpcQueue.push(new CmdCheckProtect(*Game::pLocalPlayer, MenuState.selectedPlayer));
						else if (IsInLobby())
							MenuState.lobbyRpcQueue.push(new CmdCheckProtect(*Game::pLocalPlayer, MenuState.selectedPlayer));
					}
				}
				else if ((IsHost() && IsInGame()) || !MenuState.SafeMode) {
					if (AnimatedButton("Protect")) {
						for (auto p : selectedPlayers) {
							...
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcProtectPlayer(*Game::pLocalPlayer, p, colorId));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcProtectPlayer(*Game::pLocalPlayer, p, colorId));
						}
					}
				}
				...
				/*if ((IsInGame() || IsInLobby()) && selectedPlayer.get_PlayerData()->fields.IsDead) {
					if (AnimatedButton("Revive"))
					{
						for (auto p : selectedPlayers) {
							...
							if (IsInGame()) {
								MenuState.rpcQueue.push(new RpcRevive(p.validate().get_PlayerControl()));
							}
							else if (IsInLobby()) {
								MenuState.lobbyRpcQueue.push(new RpcRevive(p.validate().get_PlayerControl()));
							}
						}
					}
				}*/
				if (selectedPlayers.size() == 1 && !selectedPlayer.is_LocalPlayer() && (IsInMultiplayerGame() || IsInLobby()) && MenuState.AprilFoolsMode) {
					if (MenuState.ChatCooldown >= 3.5f) {
						if (AnimatedButton("Mog Player [Sigma]")) {
							...
							if (IsInGame()) MenuState.rpcQueue.push(new RpcSendChat(*Game::pLocalPlayer, brainrotList[randi(0, brainrotList.size() - 1)], selectedPlayer.get_PlayerControl()));
							if (IsInLobby()) MenuState.lobbyRpcQueue.push(new RpcSendChat(*Game::pLocalPlayer, brainrotList[randi(0, brainrotList.size() - 1)], selectedPlayer.get_PlayerControl()));
							MenuState.MessageSent = true;
						}
						if (MenuState.DiddyPartyMode && AnimatedButton("Rizz Up Player [Skibidi]")) {
							...
							if (IsInGame()) MenuState.rpcQueue.push(new RpcSendChat(*Game::pLocalPlayer, rizzLinesList[randi(0, rizzLinesList.size() - 1)], selectedPlayer.get_PlayerControl()));
							if (IsInLobby()) MenuState.lobbyRpcQueue.push(new RpcSendChat(*Game::pLocalPlayer, rizzLinesList[randi(0, rizzLinesList.size() - 1)], selectedPlayer.get_PlayerControl()));
							MenuState.MessageSent = true;
						}
					}
				}
				...
				static int ventId = 0;
				if ((IsHost() || !MenuState.SafeMode) && IsInGame()) {
					...
					if (AnimatedButton("Teleport to Vent")) {
						for (auto p : selectedPlayers) {
							MenuState.rpcQueue.push(new RpcBootFromVent(p.validate().get_PlayerControl(),
								(MenuState.mapType == Settings::MapType::Hq) ? ventId + 1 : ventId)); //MiraHQ vents start from 1 instead of 0
						}
					}
				}
				...
				{
					if ((!MenuState.SafeMode || (selectedPlayer.is_LocalPlayer() && selectedPlayers.size() == 1)) && AnimatedButton("Complete all Tasks")) {
						if (MenuState.SafeMode) {
							CompleteAllTasks();
						}
						else {
					...
					if (selectedPlayers.size() == 1) {
						...
						if (MenuState.RevealRoles && PlayerIsImpostor(selectedPlayer.get_PlayerData()))
						{
						...
						bool shouldEndListBox = ImGui::ListBoxHeader("###tasks#list"/*, ImVec2(181, 94) * MenuState.dpiScale*/);
						...
						else
						{
							for (auto task : tasks)
							{
								if (task->fields.taskStep == task->fields.MaxStep)
									...
									if ((!MenuState.SafeMode || selectedPlayer.is_LocalPlayer())) {
										if (AnimatedButton((std::string(TranslateTaskTypes(task->fields._.TaskType))).c_str()))
											MenuState.taskRpcQueue.push(new RpcForceCompleteTask(selectedPlayer.get_PlayerControl(), task->fields._._Id_k__BackingField));
									}
				...
				auto it = MenuState.WarnedFriendCodes.find(WarnedfriendCode);
				int warnCount = (it != MenuState.WarnedFriendCodes.end()) ? it->second : 0;
				...
				if ((IsInLobby() || IsInMultiplayerGame()) && (!selectedPlayer.is_LocalPlayer() && selectedPlayers.size() == 1)) {
					...
					bool cooldownActive = (MenuState.NotifyWarned && (currentTime - MenuState.LastWarnTime < 3.0));
					...
					if (!cooldownActive) {
						if (ImGui::Button("Add Warn")) {
							if (strlen(warnReasonBuf) > 0) {
								...
								MenuState.WarnedFriendCodes[WarnedfriendCode] = warnCount + 1;
								MenuState.WarnReasons[WarnedfriendCode].push_back(reasonStr);
								warnReasonBuf[0] = '\0';
								MenuState.Save();

								if (MenuState.NotifyWarned) {
									MenuState.LastWarnTime = currentTime;
					...
					auto& warnReasons = MenuState.WarnReasons[WarnedfriendCode];
					if (!warnReasons.empty()) {
						...
						if (ImGui::Button("Delete")) {
							if (selectedReason >= 0 && selectedReason < (int)warnReasons.size()) {
								...
								if (--MenuState.WarnedFriendCodes[WarnedfriendCode] <= 0) {
									MenuState.WarnedFriendCodes.erase(WarnedfriendCode);
									MenuState.WarnReasons.erase(WarnedfriendCode);
								}
			...
			if (openTrolling && selectedPlayer.has_value()) {
				if ((IsHost() && IsInGame()) || !MenuState.SafeMode) {
					if (AnimatedButton("Send Blank Chat As")) {
						for (auto p : selectedPlayers) {
							if (IsInGame()) MenuState.rpcQueue.push(new RpcSendChatNote(p.validate().get_PlayerControl(), 1));
							if (IsInLobby()) MenuState.lobbyRpcQueue.push(new RpcSendChatNote(p.validate().get_PlayerControl(), 1));
						}
					}
					ImGui::SameLine();
					if (AnimatedButton("Spam Blank Chat As")) {
						for (auto p : selectedPlayers) {
							if (IsInGame()) MenuState.rpcQueue.push(new RpcSpamChatNote(p.validate().get_PlayerControl()));
							if (IsInLobby()) MenuState.lobbyRpcQueue.push(new RpcSpamChatNote(p.validate().get_PlayerControl()));
						}
					}
				}

				if ((IsHost() || !MenuState.SafeMode) && IsInGame() && selectedPlayers.size() == 1) {
					if (!MenuState.InMeeting) {
						if (AnimatedButton("Force Meeting By") && !GetPlayerData(selectedPlayer.get_PlayerControl())->fields.IsDead) {
							if (IsHost() || !MenuState.SafeMode) MenuState.rpcQueue.push(new RpcForceReportBody(selectedPlayer.get_PlayerControl(), {}));
							else {
								MenuState.rpcQueue.push(new RpcReportBody(selectedPlayer));
								MenuState.rpcQueue.push(new RpcForceMeeting(selectedPlayer.get_PlayerControl(), {}));
							}
						}
					}
					else {
				...
				if ((IsHost() || !MenuState.SafeMode) && selectedPlayer.has_value() && IsInGame() && selectedPlayers.size() == 1) {
					ImGui::SameLine();
					if (!MenuState.InMeeting) {
						if (!selectedPlayer.get_PlayerData()->fields.IsDead && AnimatedButton("Self-Report")) {
							if (IsHost() || !MenuState.SafeMode) MenuState.rpcQueue.push(new RpcForceReportBody(selectedPlayer.get_PlayerControl(), selectedPlayer));
							else {
								MenuState.rpcQueue.push(new RpcReportBody(selectedPlayer));
								MenuState.rpcQueue.push(new RpcForceMeeting(selectedPlayer.get_PlayerControl(), selectedPlayer));
							}
						}
					}
					else {
						if (AnimatedButton("Self-Report")) {
							MenuState.rpcQueue.push(new RpcForceMeeting(selectedPlayer.get_PlayerControl(), MenuState.selectedPlayer));
						}
					}
				}
				...
				if (!selectedPlayer.is_LocalPlayer() && selectedPlayers.size() == 1) {
					...
					if (outfit != NULL) {
						...
						if (!MenuState.activeImpersonation && AnimatedButton("Impersonate")) {
							if (queue != nullptr) {
						...
						ImGui::SetNextItemWidth(300 * MenuState.dpiScale);
						if (ImGui::CollapsingHeader("Cosmetics Stealer")) {
							if (AnimatedButton("Name"))
								ImpersonateName(selectedPlayer.get_PlayerData());
							ImGui::SameLine();
							if ((IsHost() || !MenuState.SafeMode) && AnimatedButton("Color") && queue != nullptr)
								queue->push(new RpcForceColor(*Game::pLocalPlayer, colorId));
							ImGui::SameLine();
						...
						if (ImGui::CollapsingHeader("Cosmetics Resetter")) {
							...
							if (AnimatedButton("Name") && queue != nullptr)
								queue->push(new RpcSetName(MenuState.originalName));
							ImGui::SameLine();
							if (AnimatedButton("Color") && queue != nullptr) {
								if (IsHost() || !MenuState.SafeMode) queue->push(new RpcForceColor(*Game::pLocalPlayer, MenuState.originalColor));
								else queue->push(new RpcSetColor(MenuState.originalColor));
							}
							ImGui::SameLine();
							if (AnimatedButton("Hat") && queue != nullptr)
								queue->push(new RpcSetHat(MenuState.originalHat));
							ImGui::SameLine();
							if (AnimatedButton("Skin") && queue != nullptr)
								queue->push(new RpcSetSkin(MenuState.originalSkin));

							if (AnimatedButton("Visor") && queue != nullptr)
								queue->push(new RpcSetVisor(MenuState.originalVisor));
							ImGui::SameLine();
							if (AnimatedButton("Pet") && queue != nullptr)
								queue->push(new RpcSetPet(MenuState.originalNamePlate));
							ImGui::SameLine();
							if (AnimatedButton("Nameplate") && queue != nullptr)
								queue->push(new RpcSetNamePlate(MenuState.originalNamePlate));
						}
					}
				}

				if (!MenuState.SafeMode && AnimatedButton("Impersonate Everyone To") && selectedPlayers.size() == 1) {
					app::NetworkedPlayerInfo_PlayerOutfit* outfit = GetPlayerOutfit(selectedPlayer.get_PlayerData());
					auto petId = outfit->fields.PetId;
					auto skinId = outfit->fields.SkinId;
					auto hatId = outfit->fields.HatId;
				...
				if (MenuState.activeImpersonation)
				{
				...
				if (!MenuState.SafeMode && IsInLobby() && AnimatedButton(selectedPlayers.size() == 1 ? "Allow Player to NoClip" : "Allow Players to NoClip")) {
					for (auto p : selectedPlayers) {
						if (p.has_value() && p.validate().is_LocalPlayer()) MenuState.NoClip = true;
						else MenuState.lobbyRpcQueue.push(new RpcMurderLoop(*Game::pLocalPlayer, p.validate().get_PlayerControl(), 1, true));
						if (selectedPlayers.size() == 1) {
				...
				if (!MenuState.SafeMode) {
					if (AnimatedButton("Suicide")) {
						for (auto p : selectedPlayers) {
							auto validPlayer = p.validate();
							if (IsInGame()) {
								MenuState.rpcQueue.push(new RpcMurderPlayer(validPlayer.get_PlayerControl(), validPlayer.get_PlayerControl(),
									validPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
							}
							else if (IsInLobby()) {
								MenuState.lobbyRpcQueue.push(new RpcMurderPlayer(validPlayer.get_PlayerControl(), validPlayer.get_PlayerControl(),
									validPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
							}
						}
					}
					...
					if (AnimatedButton("Exile")) {
						for (auto p : selectedPlayers) {
							if (IsInGame()) MenuState.rpcQueue.push(new RpcExiled(p.validate().get_PlayerControl(), true));
							else MenuState.lobbyRpcQueue.push(new RpcExiled(p.validate().get_PlayerControl(), true));
						}
					}
				}

				if ((IsHost() || !MenuState.SafeMode) && selectedPlayers.size() == 1) {
					if (IsInGame()) {
					...
					if (murderDelay <= 0) {
						if (murderCount > 0 && selectedPlayer.has_value() && !selectedPlayer.get_PlayerData()->fields.Disconnected) {
							if (IsInGame()) {
								MenuState.rpcQueue.push(new RpcMurderLoop(*Game::pLocalPlayer, selectedPlayer.get_PlayerControl(), 1, false));
							}
							else if (IsInLobby()) {
								MenuState.lobbyRpcQueue.push(new RpcMurderLoop(*Game::pLocalPlayer, selectedPlayer.get_PlayerControl(), 1, false));
							}
							murderDelay = 5;
							murderCount--;
						}
					...
					if (GetAllPlayerControl().size() == 1) {
						if (IsInGame()) {
							if (!farmLoop && AnimatedButton("Level Farm")) {
								MenuState.rpcQueue.push(new RpcSetRole(*Game::pLocalPlayer, RoleTypes__Enum::ImpostorGhost));
								farmLoop = true;
						...
						if (farmDelay <= 0) {
							if (farmCount > 0 && selectedPlayer.has_value() && !selectedPlayer.get_PlayerData()->fields.Disconnected) {
								MenuState.taskRpcQueue.push(new RpcMurderLoop(*Game::pLocalPlayer, selectedPlayer.get_PlayerControl(), 2, false));
								farmDelay = 2;
								farmCount--;
							}
							else {
								farmLoop = false;
								farmCount = 0;
								MenuState.rpcQueue.push(new RpcSetRole(*Game::pLocalPlayer, RoleTypes__Enum::Impostor));
							}
						}
						else farmDelay--;
					}
				...
				if (!MenuState.SafeMode && IsInGame() && selectedPlayers.size() == 1) {
					ImGui::SameLine();
					...
					if (suicideDelay <= 0) {
						if (suicideCount > 0 && selectedPlayer.has_value() && !selectedPlayer.get_PlayerData()->fields.Disconnected) {
							if (IsInGame()) {
								MenuState.rpcQueue.push(new RpcMurderPlayer(selectedPlayer.get_PlayerControl(), selectedPlayer.get_PlayerControl(),
									selectedPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
							}
							else if (IsInLobby()) {
								MenuState.lobbyRpcQueue.push(new RpcMurderPlayer(selectedPlayer.get_PlayerControl(), selectedPlayer.get_PlayerControl(),
									selectedPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
							}
							suicideDelay = 15;
							suicideCount--;
				...
				if (!MenuState.SafeMode && selectedPlayers.size() == 1 && IsInGame()) {
					if (AnimatedButton("Kill Crewmates By")) {
						for (auto player : GetAllPlayerControl()) {
							if (!PlayerIsImpostor(GetPlayerData(player))) {
								if (!MenuState.SafeMode) {
									if (IsInGame()) {
										MenuState.rpcQueue.push(new RpcMurderPlayer(selectedPlayer.get_PlayerControl(), player,
											selectedPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
									}
									else if (IsInLobby()) {
										MenuState.lobbyRpcQueue.push(new RpcMurderPlayer(selectedPlayer.get_PlayerControl(), player,
											selectedPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
									}
								}
								else {
									if (IsInGame()) {
										MenuState.rpcQueue.push(new FakeMurderPlayer(selectedPlayer.get_PlayerControl(), player,
											selectedPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
									}
									else if (IsInLobby()) {
										MenuState.lobbyRpcQueue.push(new FakeMurderPlayer(selectedPlayer.get_PlayerControl(), player,
											selectedPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || MenuState.BypassAngelProt));
									}
								}
							}
				...
				if (!MenuState.SafeMode)
				{
					if (selectedPlayers.size() == 1 && AnimatedButton("Shift Everyone To"))
					{
						for (auto player : GetAllPlayerControl()) {
							...
							if (IsInGame()) {
								MenuState.rpcQueue.push(new RpcShapeshift(player, MenuState.selectedPlayer, !MenuState.AnimationlessShapeshift));
							}
							else if (IsInLobby()) {
								MenuState.lobbyRpcQueue.push(new RpcShapeshift(player, MenuState.selectedPlayer, !MenuState.AnimationlessShapeshift));
							}
						}
					}
					ImGui::SameLine();
					if (AnimatedButton("Unshift Everyone"))
					{
						for (auto player : GetAllPlayerControl()) {
							if (IsInGame()) {
								MenuState.rpcQueue.push(new RpcShapeshift(player, PlayerSelection(player), !MenuState.AnimationlessShapeshift));
							}
							else if (IsInLobby()) {
								MenuState.lobbyRpcQueue.push(new RpcShapeshift(player, PlayerSelection(player), !MenuState.AnimationlessShapeshift));
							}
						}
					}
						...
						if (roleType == RoleTypes__Enum::Phantom) {
							if (AnimatedButton("Force Vanish"))
							{
								for (auto p : selectedPlayers) {
									auto validPlayer = p.validate();
									if (IsInGame()) {
										MenuState.rpcQueue.push(new RpcVanish(validPlayer.get_PlayerControl()));
									}
									else if (IsInLobby()) {
										MenuState.lobbyRpcQueue.push(new RpcVanish(validPlayer.get_PlayerControl()));
									}
								}
							}
							ImGui::SameLine();
							if (AnimatedButton("Force Appear"))
							{
								for (auto p : selectedPlayers) {
									auto validPlayer = p.validate();
									if (IsInGame()) {
										MenuState.rpcQueue.push(new RpcVanish(validPlayer.get_PlayerControl(), true));
									}
									else if (IsInLobby()) {
										MenuState.lobbyRpcQueue.push(new RpcVanish(validPlayer.get_PlayerControl(), true));
									}
								}
							}
						}
				...
				ImGui::NewLine();
				if ((IsHost() || !MenuState.SafeMode) && MenuState.InMeeting && selectedPlayers.size() == 1) {
					if (AnimatedButton("Vote Off")) {
						MenuState.VoteOffPlayerId = selectedPlayer.get_PlayerControl()->fields.PlayerId;
						for (auto player : GetAllPlayerControl()) {
							MenuState.rpcQueue.push(new RpcVotePlayer(player, selectedPlayer.get_PlayerControl()));
						}
					}
				}
				...
				if (!selectedPlayer.is_LocalPlayer() && selectedPlayers.size() == 1) {
					if (AnimatedButton("Teleport To")) {
						if (IsInGame())
							MenuState.rpcQueue.push(new RpcSnapTo(GetTrueAdjustedPosition(selectedPlayer.get_PlayerControl())));
						else if (IsInLobby())
							MenuState.lobbyRpcQueue.push(new RpcSnapTo(GetTrueAdjustedPosition(selectedPlayer.get_PlayerControl())));
					}
				}
				ImGui::SameLine();
				if (!selectedPlayer.is_LocalPlayer() && !MenuState.SafeMode) {
					if (AnimatedButton("Teleport To You")) {
						for (auto p : selectedPlayers) {
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcForceSnapTo(p.validate().get_PlayerControl(), GetTrueAdjustedPosition(*Game::pLocalPlayer)));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcForceSnapTo(p.validate().get_PlayerControl(), GetTrueAdjustedPosition(*Game::pLocalPlayer)));
						}
					}
				}
				...
				{
					if (MenuState.ActiveAttach && selectedPlayer.has_value() && (MenuState.playerToAttach.equals(MenuState.selectedPlayer) || selectedPlayer.is_LocalPlayer())) {
						if (AnimatedButton(MenuState.AprilFoolsMode ? "Stop Backshotting" : "Stop Attaching")) {
							MenuState.playerToAttach = {};
							MenuState.ActiveAttach = false;
						}
					}
					else {
						if (!selectedPlayer.is_LocalPlayer() && AnimatedButton(MenuState.AprilFoolsMode ? "Backshot To" : "Attach To")) {
							MenuState.playerToAttach = MenuState.selectedPlayer;
							MenuState.ActiveAttach = true;
						}
					}
				}

				if ((IsHost() || !MenuState.SafeMode) && (IsInGame() || IsInLobby()) && selectedPlayers.size() == 1 && !selectedPlayer.get_PlayerData()->fields.IsDead) {
					if (AnimatedButton("Turn into Ghost"))
					{
						if (PlayerIsImpostor(selectedPlayer.get_PlayerData())) {
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcSetRole(selectedPlayer.get_PlayerControl(), RoleTypes__Enum::ImpostorGhost));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcSetRole(selectedPlayer.get_PlayerControl(), RoleTypes__Enum::ImpostorGhost));
						}
						else {
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcSetRole(selectedPlayer.get_PlayerControl(), RoleTypes__Enum::CrewmateGhost));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcSetRole(selectedPlayer.get_PlayerControl(), RoleTypes__Enum::CrewmateGhost));
						}
					}
				}

				if ((IsHost() || !MenuState.SafeMode) && (IsInGame() || IsInLobby()) && selectedPlayers.size() == 1) {
					if (!IsInMultiplayerGame() || !selectedPlayer.get_PlayerControl()->fields.roleAssigned)
					{
						...
						{
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcSetRole(selectedPlayer.get_PlayerControl(), RoleTypes__Enum(MenuState.FakeRole)));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcSetRole(selectedPlayer.get_PlayerControl(), RoleTypes__Enum(MenuState.FakeRole)));
						}
					}
					else {
						static int ghostRole = 0;
						if (CustomListBoxInt("Select Role", &ghostRole, GHOSTROLES, 100.0f * MenuState.dpiScale))
							MenuState.Save();
						ImGui::SameLine();
						if (AnimatedButton("Set Role"))
						{
							...
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcSetRole(selectedPlayer.get_PlayerControl(), roleType));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcSetRole(selectedPlayer.get_PlayerControl(), roleType));
						}
					}
				}

				if (GameOptions().GetBool(BoolOptionNames__Enum::VisualTasks)) {
					if (!MenuState.SafeMode && AnimatedButton("Set Scanner")) {
						for (auto p : selectedPlayers) {
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcForceScanner(p.validate().get_PlayerControl(), true));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcForceScanner(p.validate().get_PlayerControl(), true));
						}
					}
					ImGui::SameLine();
					if (!MenuState.SafeMode && AnimatedButton("Stop Scanner")) {
						for (auto p : selectedPlayers) {
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcForceScanner(p.validate().get_PlayerControl(), false));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcForceScanner(p.validate().get_PlayerControl(), false));
						}
					}
				}
				ImGui::NewLine();

				if (MenuState.selectedPlayers.size() == 1) {
					if ((IsInGame() || IsInLobby()) && !selectedPlayer.is_Disconnected() && !selectedPlayer.is_LocalPlayer())
					{
						if (MenuState.playerToWhisper.equals(MenuState.selectedPlayer) && MenuState.activeWhisper) {
							if (AnimatedButton("Stop Whispering To")) {
								MenuState.playerToWhisper = {};
								MenuState.activeWhisper = false;
							}
						}
						else {
							if (AnimatedButton("Whisper To")) {
								MenuState.playerToWhisper = MenuState.selectedPlayer;
								MenuState.activeWhisper = true;
							}
						}
					}
					//we have to send these rpc messages as ourselves since anticheat only allows you to send rpcs with your own net id
					if (AnimatedButton(!MenuState.SafeMode ? "Force AUM Detection" : "Fake AUM Detection")) {
						if (IsInGame()) MenuState.rpcQueue.push(new RpcForceDetectAum(selectedPlayer, !MenuState.SafeMode));
						if (IsInLobby()) MenuState.lobbyRpcQueue.push(new RpcForceDetectAum(selectedPlayer, !MenuState.SafeMode));
					}
					ImGui::SameLine();
					static std::string aumMessage = "";
					if (AnimatedButton(!MenuState.SafeMode ? "Force SickoChat" : "Fake SickoChat")) {
						if (IsInGame()) MenuState.rpcQueue.push(new RpcForceSickoChat(selectedPlayer, aumMessage, !MenuState.SafeMode));
						if (IsInLobby()) MenuState.lobbyRpcQueue.push(new RpcForceSickoChat(selectedPlayer, aumMessage, !MenuState.SafeMode));
					}

					InputString("AUM Message", &aumMessage);

					if (!MenuState.SafeMode && (IsInGame() || IsInLobby()) && !selectedPlayer.is_Disconnected() && !selectedPlayer.is_LocalPlayer())
					{
						if (MenuState.playerToChatAs.equals(MenuState.selectedPlayer) && MenuState.activeChatSpoof) {
							if (AnimatedButton("Stop Chatting As")) {
								MenuState.playerToChatAs = {};
								MenuState.activeChatSpoof = false;
							}
						}
						else {
							if (AnimatedButton("Chat As")) {
								MenuState.playerToChatAs = MenuState.selectedPlayer;
								MenuState.activeChatSpoof = true;
							}
						}
					}
				}
			...
			if (openInfo && selectedPlayer.has_value() && selectedPlayers.size() == 1 && !selectedPlayer.get_PlayerControl()->fields.notRealPlayer) {
				...
				if (AnimatedButton("Steal Data")) {
					MenuState.StealedPUID = convert_from_string(selectedPlayer.get_PlayerData()->fields.Puid);
					MenuState.StealedFC = convert_from_string(selectedPlayer.get_PlayerData()->fields.FriendCode);
					MenuState.Save();
				}
				ImGui::Dummy(ImVec2(15, 15) * MenuState.dpiScale);
				if (InputString("PUID", &MenuState.StealedPUID)) {
					MenuState.Save();
				}
				ImGui::Dummy(ImVec2(2, 2) * MenuState.dpiScale);
				if (InputString("Friend Code", &MenuState.StealedFC)) {
					MenuState.Save();
				}
				ImGui::Dummy(ImVec2(10, 10) * MenuState.dpiScale);
				{
				...
				if (AnimatedButton("Report Player")) {
					if (IsInGame()) MenuState.rpcQueue.push(new ReportPlayer(selectedPlayer.get_PlayerControl(), (ReportReasons__Enum)reportReason));
					if (IsInLobby()) MenuState.lobbyRpcQueue.push(new ReportPlayer(selectedPlayer.get_PlayerControl(), (ReportReasons__Enum)reportReason));
				}
				...
				if (MenuState.TempBanEnabled && IsHost() && !selectedPlayer.is_LocalPlayer()) {
					...
					if (ImGui::Button("Confirm TempBan")) {
						...
						if (!targetFC.empty() && targetFC != selfFC) {
							...
							if (totalSeconds > 0) {
								...
								MenuState.TempBannedFCs[targetFC] = banEnd;
								MenuState.Save();
				...
				if ((IsHost() || !MenuState.SafeMode) && (IsInGame() || IsInLobby()) && selectedPlayers.size() == 1) {
					...
					{
						if (IsInGame())
							MenuState.rpcQueue.push(new RpcForceName(selectedPlayer.get_PlayerControl(), forcedName));
						else if (IsInLobby())
							MenuState.lobbyRpcQueue.push(new RpcForceName(selectedPlayer.get_PlayerControl(), forcedName));
					}

					CustomListBoxInt(" ", &forcedColor, COLORS, 85.0f * MenuState.dpiScale);
					ImGui::SameLine();
					if (AnimatedButton("Force Color"))
					{
						if (IsInGame()) {
							if (IsHost())
								MenuState.rpcQueue.push(new RpcForceColor(selectedPlayer.get_PlayerControl(), forcedColor));
							else
								MenuState.rpcQueue.push(new RpcForceColor(selectedPlayer.get_PlayerControl(), forcedColor));
						}
						else if (IsInLobby()) {
							if (IsHost())
								MenuState.lobbyRpcQueue.push(new RpcForceColor(selectedPlayer.get_PlayerControl(), forcedColor));
							else
								MenuState.lobbyRpcQueue.push(new RpcForceColor(selectedPlayer.get_PlayerControl(), forcedColor));
						}
					}
					...
					if (!MenuState.SafeMode && (IsInGame() || IsInLobby())) {
						...
						if (AnimatedButton("Force Level")) {
							if (IsInGame())
								MenuState.rpcQueue.push(new RpcSetLevel(selectedPlayer.get_PlayerControl(), level));
							else if (IsInLobby())
								MenuState.lobbyRpcQueue.push(new RpcSetLevel(selectedPlayer.get_PlayerControl(), level));
						}
					}
				}
			}
		...
		if (openPlayer || MenuState.selectedPlayers.size() == 0 || IsInLobby()) { //clear murder/suicide loops
			murderLoop = false;
			suicideLoop = false;
			murderCount = 0;
			suicideCount = 0;
...
#pragma region Blinking tab? No more!
		...
		if (MenuState.BlinkPlayersTab && !isBlinking) {
			blinkDelay = 5;
			isBlinking = true;
		}
		if (isBlinking && MenuState.BlinkPlayersTab) {
