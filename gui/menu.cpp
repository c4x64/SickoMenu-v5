...
namespace Menu {
	...
	void Init() {
		ImGui::SetNextWindowSize(ImVec2(600, 450) * MenuState.dpiScale, ImGuiCond_None);
	...
	void Render() {
		try {
			if (!init)
				Menu::Init();
			std::string modText = std::format("SickoMenu {}", MenuState.SickoVersion);
			ImGui::Begin("SickoMenu", &MenuState.ShowMenu, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollWithMouse);
			...
			/*if (MenuState.AprilFoolsMode) {
				...
				if (MenuState.DiddyPartyMode) ImGui::TextColored(DiddyCol, " [Diddy Party Mode]");
				else ImGui::TextColored(DiddyCol, IsChatCensored() || IsStreamerMode() ? " [F***son Mode]" : " [Fuckson Mode]");
			}*/
			ImGui::SameLine(ImGui::GetWindowWidth() - 19 * MenuState.dpiScale);
			if (AnimatedButton("-")) MenuState.ShowMenu = false; //minimize button
			//ImGui::BeginTabBar("AmongUs#TopBar", ImGuiTabBarFlags_NoTabListScrollingButtons);
			ImGui::BeginChild("###SickoMenu", ImVec2(90 * MenuState.dpiScale, 0), true, ImGuiWindowFlags_NoBackground);
			// Search field
			ImGui::SetNextItemWidth(90 * MenuState.dpiScale); // Adjust the width of the input box
			if (InputStringWithHint("##Search", "Search...", &searchQuery)/* && MenuState.AprilFoolsMode*/) {
				/*if (ToLower(searchQuery) == StrRev("nosduh")) {
					MenuState.AprilFoolsMode = !MenuState.AprilFoolsMode;
					if (!MenuState.AprilFoolsMode) MenuState.DiddyPartyMode = false;
				}
				if (ToLower(searchQuery) == StrRev("yddid")) {
					MenuState.DiddyPartyMode = !MenuState.DiddyPartyMode;
				}*/
			}
			...
			if ((IsInGame() && !MenuState.mapDoors.empty()) && ImGui::Selectable("Doors", openDoors)) {
...
#ifdef _DEBUG
			if (MenuState.showDebugTab && ImGui::Selectable("Debug", openDebug)) {
			...
			if (!isPanicWarning) {
				ImGui::SetCursorPos(ImVec2(ImGui::GetWindowWidth() - 90 * MenuState.dpiScale, ImGui::GetWindowHeight() - 20 * MenuState.dpiScale));
				if (/*!MenuState.AprilFoolsMode && */ColoredButton(PanicCol, "Disable Menu")) {
					isPanicWarning = MenuState.PanicWarning;
					if (!MenuState.PanicWarning) MenuState.PanicMode = true;
				}
				/*if (MenuState.AprilFoolsMode && ColoredButton(DiddyCol,MenuState.DiddyPartyMode ? "Rizz Up Diddy" : 
				...
				bool panicKeybind = MenuState.KeyBinds.Toggle_Sicko != 0x00;
				ImGui::SetCursorPos(ImVec2(ImGui::GetWindowWidth() - 90 * MenuState.dpiScale,
					ImGui::GetWindowHeight() - 65 * MenuState.dpiScale));
				if (!panicKeybind) {
					ImGui::TextColored(PanicCol, "No Panic");
					ImGui::TextColored(PanicCol, "Keybind!");
				}
				else {
					ImGui::TextColored(PanicCol, ("Press " + (std::string)KeyBinds::ToString(MenuState.KeyBinds.Toggle_Sicko)).c_str());
			...
			if (openDoors) {
				if (IsInGame() && !MenuState.mapDoors.empty()) DoorsTab::Render();
				else {
					CloseAllOtherTabs(Tabs::Game);
					GameTab::Render();
				}
...
#ifdef _DEBUG
			if (openDebug) {
				if (MenuState.showDebugTab) DebugTab::Render();
				else {
					CloseAllOtherTabs(Tabs::Game);
					GameTab::Render();
				}
