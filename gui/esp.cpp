...
{
	if (text.empty() || MenuState.PanicMode) return;
	ImVec2 ImScreen = pos;
	if (centered)
	{
	...
	if (outlined)
	{
		CurrentWindow->DrawList->AddText(nullptr, 0.f,
			ImScreen + 0.5f * MenuState.dpiScale,
...
static void RenderLine(const ImVec2& start, const ImVec2& end, const ImVec4& color, bool shadow = false) noexcept
{
	...
	if (shadow)
	{
		CurrentWindow->DrawList->AddLine(
			start + 0.5f * MenuState.dpiScale,
			end + 0.5f * MenuState.dpiScale,
			ImGui::GetColorU32(color) & IM_COL32_A_MASK, 1.0f * MenuState.dpiScale);
	}

	CurrentWindow->DrawList->AddLine(start, end, ImGui::GetColorU32(color), 1.0f * MenuState.dpiScale);
}
...
static void RenderBox(const ImVec2& top, const ImVec2& bottom, const float height, const float width, const ImVec4& color, const bool wantsShadow = true)
{
	...
	const ImVec2 points[] = {
		...
		{ bottom.x - 0.5f * MenuState.dpiScale, bottom.y }, { ((float)(int)(bottom.x * 0.75f + top.x * 0.25f)), bottom.y },

		{ top.x + 0.5f * MenuState.dpiScale, bottom.y }, { ((float)(int)(top.x * 0.75f + bottom.x * 0.25f)), bottom.y },
		{ top.x, bottom.y }, { top.x, ((float)(int)(bottom.y * 0.75f + top.y * 0.25f)) },

		{ bottom.x, top.y }, { bottom.x, ((float)(int)(top.y * 0.75f + bottom.y * 0.25f)) },
		{ bottom.x - 0.5f * MenuState.dpiScale, top.y }, { ((float)(int)(bottom.x * 0.75f + top.x * 0.25f)), top.y },

		top, { ((float)(int)(top.x * 0.75f + bottom.x * 0.25f)), top.y },
		{ top.x, top.y + 0.5f * MenuState.dpiScale }, { top.x, ((float)(int)(top.y * 0.75f + bottom.y * 0.25f)) }
	};
	...
	if (wantsShadow) {
		...
		for (size_t i = 0; i < std::size(points); i += 2) {
			RenderLine(points[i] + 0.5f * MenuState.dpiScale, points[i + 1] + 0.5f * MenuState.dpiScale, shadowColor, false);
		}
	}
	for (size_t i = 0; i < std::size(points); i += 2) {
		...
		if (renderPlayerEsp) {
			for (auto& it : instance.m_Players)
			{
				if (const auto& player = it.playerData.validate();
					...
					&& (!player.get_PlayerData()->fields.IsDead || MenuState.ShowEsp_Ghosts)
					&& it.OnScreen)
				{
					if (MenuState.ShowEsp_Box)
					{
					...
					// TODO: show x and y
					if (MenuState.ShowEsp_Distance)
					{
						const ImVec2 position{ it.Position.x, it.Position.y + 15.0f * MenuState.dpiScale };
		...
		for (auto& it : instance.m_Players)
		{
				...
				&& it.OnScreen)
			{
				...
				if (MenuState.ShowEsp_Distance)
				{
					...
					ImVec2 position = { it.Position.x, it.Position.y + 15.0f * MenuState.dpiScale };
					ImVec2 position2 = { it.Position.x, it.Position.y + 30.0f * MenuState.dpiScale };
					...
					float offset = 15.0f * MenuState.dpiScale;
