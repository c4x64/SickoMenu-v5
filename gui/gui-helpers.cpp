...
bool CustomListBoxInt(const char* label, int* value, const std::vector<const char*> list, float width, ImVec4 col, ImGuiComboFlags flags, const char* visualLabel) {
	...
	PushItemWidth(width);
	bool response = ImGui::BeginCombo(comboLabel.c_str(), (*value >= 0 ? list.at(*value) : nullptr), ImGuiComboFlags_NoArrowButton | flags);
	if (response) {
		response = false;
	...
	ImGui::SameLine(0, spacing);
	//noobuild by gdjkhp
	if (col.x == 0 && col.y == 0 && col.z == 0 && col.w == 0) ImGui::Text(visualLabel == "" ? label : visualLabel);
	else ImGui::TextColored(col, visualLabel == "" ? label : visualLabel);
...
bool CustomListBoxIntMultiple(const char* label, std::vector<std::pair<const char*, bool>>* list, float width, bool resetButton, ImGuiComboFlags flags) {
	...
	if (countSelected > 0) {
		...
		preview = label;
	bool response = ImGui::BeginCombo(comboLabel.c_str(), preview.c_str(), flags);
	if (response) {
		response = false;
		for (auto& pair : *list) {
			if (strcmp(pair.first, "") == 0) // ignore all entries with empty labels so we can create padding
				continue;
			if (ImGui::Selectable(pair.first, pair.second)) {
				pair.second ^= 1;
				response = true;
			}
			if (pair.second)
...
bool CustomListBoxPlayerSelectionMultiple(const char* label, std::array<std::pair<PlayerSelection, bool>, Game::MAX_PLAYERS>* list, float width, bool resetButton, ImGuiComboFlags flags) {
	...
	if (response) {
		...
		for (auto playerData : GetAllPlayerData()) {
			...
			if (!player.has_value())
				continue;
			if (player.is_Disconnected()) // maybe make that an option for replays ? (parameter based on "MenuState.showDisconnected" related data)
				continue;
			...
			if (ImGui::Selectable(std::string("##" + playerName + "_ConsoleName").c_str(), item.second))
			{
				item.second ^= 1;
				if (item.second)
				{
			...
			if (item.second)
				SetItemDefaultFocus();
			ImGui::SameLine();
			ImGui::ColorButton(std::string("##" + playerName + "_ConsoleColorButton").c_str(), AmongUsColorToImVec4(GetPlayerColor(outfit->fields.ColorId)), ImGuiColorEditFlags_NoBorder | ImGuiColorEditFlags_NoTooltip);
			ImGui::SameLine();
			ImGui::PopStyleVar(2);
			...
			//else if (PlayerSelection(playerData).is_LocalPlayer() || std::count(MenuState.aumUsers.begin(), MenuState.aumUsers.end(), playerData->fields.PlayerId))
...
bool SliderChrono(const char* label, void* p_data, const void* p_min, const void* p_max, std::string_view format, ImGuiSliderFlags flags)
{
	...
	if (!MenuState.Replay_IsLive && !MenuState.Replay_IsPlaying) {
		...
		{
			MenuState.Replay_IsPlaying = true;
		}
	}
	else {
		// Live or Playing
		...
		{
			MenuState.Replay_IsPlaying = MenuState.Replay_IsLive = false;
		}
	}

	ImGui::SameLine(0.0f * MenuState.dpiScale, 1.0f * MenuState.dpiScale);
	...
	// Draw frame
	auto frameMin = ImVec2(frame_bb.Min.x + 7.f * MenuState.dpiScale, frame_bb.Min.y + 4.f * MenuState.dpiScale);

	const ImU32 frame_col = GetColorU32(g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
	RenderNavHighlight(frame_bb, id);
	RenderFrame(frameMin, frame_bb.Max - 4.f * MenuState.dpiScale, frame_col, true, g.Style.FrameRounding);
	...
	if (value_changed) {
		...
		auto newMatchCurrent = std::chrono::time_point_cast<std::chrono::seconds>(MenuState.MatchCurrent);
		auto matchLiveMs = std::chrono::time_point_cast<std::chrono::seconds>(MenuState.MatchLive);
	...
	if (grab_bb.Max.x > grab_bb.Min.x) {
		...
		float knob_radius = 8.0f * MenuState.dpiScale;
		...
		window->DrawList->AddRectFilled(knob_min, knob_max,
			...
			knob_radius * MenuState.RoundingRadiusMultiplier);

		if (&p_data != &p_min) {
			ImVec2 maxActive = ImVec2(grab_bb.Max.x - 14 * MenuState.dpiScale,
				grab_bb.Max.y - 2.f * MenuState.dpiScale);
			RenderFrame(frameMin, maxActive,
				GetColorU32(ImGuiCol_ButtonActive),
				true,
				g.Style.FrameRounding* MenuState.RoundingRadiusMultiplier);
		}
	}
	...
	ImGui::SameLine(0.0f * MenuState.dpiScale, 10.0f * MenuState.dpiScale);

	ImU32 liveColor = (MenuState.Replay_IsLive) ? ColorConvertFloat4ToU32(ImVec4(255.0f, 0.f, 0.f, 255.0f)) : ColorConvertFloat4ToU32(ImVec4(128.f, 128.f, 128.f, 255.0f));
	const ImVec2 circlePos(window->DC.CursorPos.x, window->DC.CursorPos.y + 9.5f * MenuState.dpiScale);
	float radius = 5.0f * MenuState.dpiScale;
	...
	window->DrawList->AddRectFilled(rect_min, rect_max,
		liveColor,
		radius * MenuState.RoundingRadiusMultiplier);
	ImGui::SameLine(0.0f * MenuState.dpiScale, 18.f * MenuState.dpiScale);
	ImGui::Text("Live");
...
bool HotKey(uint8_t& key)
{
	...
	if (ImGui::IsItemHovered()) ImGui::SetTooltip("Press any key while clicking on the keybind to change it, ESC to reset");
...
void drawPlayerDot(PlayerControl* player, const ImVec2& winPos, ImU32 color, ImU32 statusColor)
{
	...
	const ImVec2& center = ImVec2(radX, radY) * MenuState.dpiScale + winPos;

	if ((MenuState.RevealRoles || PlayerIsImpostor(GetPlayerData(*Game::pLocalPlayer))) && PlayerIsImpostor(GetPlayerData(player))) {
		ImVec2 topLeft = ImVec2(center.x - 4.5F * MenuState.dpiScale, center.y - 4.5F * MenuState.dpiScale);
		ImVec2 bottomRight = ImVec2(center.x + 4.5F * MenuState.dpiScale, center.y + 4.5F * MenuState.dpiScale);
		...
		drawList->AddCircleFilled(center, 4.5F * MenuState.dpiScale, color);
		drawList->AddCircle(center, (4.5F + 0.5F) * MenuState.dpiScale, statusColor, 0, 2.0F);
	}
}
...
void drawPlayerIcon(PlayerControl* player, const ImVec2& winPos, ImU32 color)
{
	...
	const ImVec2& p_min = ImVec2(radX, radY) * MenuState.dpiScale + winPos;
	const ImVec2& p_max = ImVec2(radXMax, radYMax) * MenuState.dpiScale + winPos;
	...
	drawList->AddImage((void*)visor.iconImage.shaderResourceView,
		...
		ImVec2(0.0f, 1.0f),
		ImVec2(1.0f, 0.0f),
		(/*MenuState.RadarVisorRoleColor && */MenuState.RevealRoles) ?
...
void drawDeadPlayerDot(DeadBody* deadBody, const ImVec2& winPos, ImU32 color)
{
	...
	drawList->AddText(GetFont(), 16 * MenuState.dpiScale,
		ImVec2(radX - 5.F, radY - 6.75F) * MenuState.dpiScale + winPos, color, "X");
}
...
void drawDeadPlayerIcon(DeadBody* deadBody, const ImVec2& winPos, ImU32 color)
{
	...
	drawList->AddImage((void*)icon.iconImage.shaderResourceView,
		ImVec2(radX, radY) * MenuState.dpiScale + winPos,
		ImVec2(radXMax, radYMax) * MenuState.dpiScale + winPos,
...
bool ToggleButton(const char* str_id, bool* v) {
	if (MenuState.DisableAnimations) {
	...
	float animation_speed = 5.0f * MenuState.AnimationSpeed; // in units per second
	if (t != target) {
	...
	float rounding = radius * MenuState.RoundingRadiusMultiplier;
	...
	float knob_rounding = (radius - 1.5f) * MenuState.RoundingRadiusMultiplier;
...
bool SickoTabGroup(const char* label, bool highlight)
{
	static ImVec4 vec = MenuState.MenuThemeColor;
	if (MenuState.RgbMenuTheme)
		vec = MenuState.RgbColor;
	else
		vec = MenuState.GradientMenuTheme ? MenuState.MenuGradientColor : MenuState.MenuThemeColor;
	auto defaultCol = ImVec4((float)(vec.x / 1.25), (float)(vec.y / 1.25), (float)(vec.z / 1.25), 0.76f * MenuState.MenuThemeColor.w);
	auto hoveredCol = ImVec4((float)(vec.x / 1.25), (float)(vec.y / 1.25), (float)(vec.z / 1.25), 0.86f * MenuState.MenuThemeColor.w);
	auto activeCol = ImVec4((float)(vec.x / 1.25), (float)(vec.y / 1.25), (float)(vec.z / 1.25), 1.0f * MenuState.MenuThemeColor.w);
...
bool ColoredButton(ImVec4 col, const char* label) {
	auto hoveredCol = ImVec4((float)(col.x / 1.25), (float)(col.y / 1.25), (float)(col.z / 1.25), (MenuState.LightMode ? 0.3f : 0.86f) * MenuState.MenuThemeColor.w);
	auto activeCol = ImVec4((float)(col.x / 1.25), (float)(col.y / 1.25), (float)(col.z / 1.25), (MenuState.LightMode ? 0.4f : 1.f) * MenuState.MenuThemeColor.w);
...
bool AnimatedButton(const char* label, const ImVec2& size) {
	if (MenuState.DisableAnimations) return ImGui::Button(label, size);
	...
	if (pulseAnimState.active) {
		pulseAnimState.t += ImGui::GetIO().DeltaTime * MenuState.AnimationSpeed / 0.2f;
