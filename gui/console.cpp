...
#undef ADD_EVENT
	...
	bool init = false;
	void Init() {
		ImGui::SetNextWindowSize(ImVec2(520, 320) * MenuState.dpiScale, ImGuiCond_None);
	...
	void Render() {
		ConsoleGui::Init();
		ImGui::Begin("###Console", &::MenuState.ShowConsole, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar);
		...
		if (AnimatedButton("-")) ::MenuState.ShowConsole = false; //minimize button
		ImGui::BeginChild("console#filter", ImVec2(520, 40) * ::MenuState.dpiScale, true, ImGuiWindowFlags_NoBackground);
		ImGui::Text("Event Filter: ");
		ImGui::SameLine();
		CustomListBoxIntMultiple("Event Types", &ConsoleGui::event_filter, 100.f * ::MenuState.dpiScale);
		if (IsInGame()) {
			ImGui::SameLine(0.f * ::MenuState.dpiScale, 5.f * ::MenuState.dpiScale);
			ImGui::Text("Player Filter: ");
			ImGui::SameLine();
			CustomListBoxPlayerSelectionMultiple("Players", &ConsoleGui::player_filter, 150.f * ::MenuState.dpiScale);
		}
		if (AnimatedButton("Clear Console")) {
			synchronized(Replay::replayEventMutex) {
				::MenuState.liveConsoleEvents.clear();
			}
		}
		ImGui::EndChild();
		ImGui::Separator();
		ImGui::BeginChild("console#scroll", ImVec2(511, 270) * ::MenuState.dpiScale, true, ImGuiWindowFlags_AlwaysVerticalScrollbar | ImGuiWindowFlags_NoBackground);
		...
		synchronized(Replay::replayEventMutex) {
			size_t i = MenuState.liveConsoleEvents.size() - 1;
			for (auto rit = MenuState.liveConsoleEvents.rbegin(); rit != MenuState.liveConsoleEvents.rend(); ++rit, --i) {
				EventInterface* evt = (*rit).get();
				if (evt == NULL)
				{
					STREAM_ERROR("MenuState.liveConsoleEvents[" << i << "] was NULL (liveConsoleEvents.size(): " << MenuState.liveConsoleEvents.size() << ")");
					continue;
				}
