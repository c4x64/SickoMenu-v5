...
ImVec4 HI(float v) {
	...
	return ImVec4(vec.x, vec.y, vec.z, v * MenuState.MenuThemeColor.w);
}
...
ImVec4 MED(float v) {
	...
	if (MenuState.RgbMenuTheme)
		...
		vec = MenuState.GradientMenuTheme ? MenuState.MenuGradientColor : MenuState.MenuThemeColor;
	return ImVec4((float)(vec.x / 1.25), (float)(vec.y / 1.25), (float)(vec.z / 1.25), v * MenuState.MenuThemeColor.w);
}
...
ImVec4 LOW(float v) {
	...
	if (MenuState.RgbMenuTheme)
		...
		vec = MenuState.GradientMenuTheme ? MenuState.MenuGradientColor : MenuState.MenuThemeColor;
	return ImVec4((float)(vec.x / 1.5625), (float)(vec.y / 1.5625), (float)(vec.z / 1.5625), v * MenuState.MenuThemeColor.w);
}

ImVec4 BG(float bg, float v = 1) {
	if (!MenuState.MatchBackgroundWithTheme) return ImVec4(bg, bg, bg, v * MenuState.MenuThemeColor.w);
	else {
		static ImVec4 vec = MenuState.MenuThemeColor;
		if (MenuState.RgbMenuTheme)
			vec = MenuState.RgbColor;
			...
			vec = MenuState.GradientMenuTheme ? MenuState.MenuGradientColor : MenuState.MenuThemeColor;
		return ImVec4((float)(vec.x / 2), (float)(vec.y / 2), (float)(vec.z / 2), v * MenuState.MenuThemeColor.w);
	}
}

#define IMGUI_TEXT(v) MenuState.LightMode ? ImVec4(0.2f, 0.2f, 0.2f, v * MenuState.MenuThemeColor.w) : ImVec4(1.f, 1.f, 1.f, v * MenuState.MenuThemeColor.w)
...
void ApplyTheme()
{
	...
	style.WindowRounding = 8.0f * MenuState.RoundingRadiusMultiplier;
	style.FramePadding = ImVec2(5, 2);
	style.FrameRounding = 6.0f * MenuState.RoundingRadiusMultiplier;
	style.ItemSpacing = ImVec2(7, 1);
	style.ItemInnerSpacing = ImVec2(1, 1);
	style.TouchExtraPadding = ImVec2(0, 0);
	style.IndentSpacing = 6.0f;
	style.ScrollbarSize = 12.0f;
	style.ScrollbarRounding = 16.0f * MenuState.RoundingRadiusMultiplier;
	style.GrabMinSize = 20.0f;
	style.GrabRounding = 2.0f * MenuState.RoundingRadiusMultiplier;
	...
	// scale by dpi
	style.ScaleAllSizes(MenuState.dpiScale);

	static int rgbDelay = 0;
	if (rgbDelay <= 0) {
		MenuState.RgbNameColor += 0.025f;
		constexpr auto tau = 2.f * 3.14159265358979323846f;
		while (MenuState.RgbNameColor > tau) MenuState.RgbNameColor -= tau;
		const auto calculate = [](float value) {return std::sin(value) * .5f + .5f; };
		auto color_r = calculate(MenuState.RgbNameColor + 0.f);
		auto color_g = calculate(MenuState.RgbNameColor + 4.f);
		auto color_b = calculate(MenuState.RgbNameColor + 2.f);
		MenuState.rgbCode = std::format("<#{:02x}{:02x}{:02x}>", int(color_r * 255), int(color_g * 255), int(color_b * 255));

		MenuState.RgbColor.x = color_r;
		MenuState.RgbColor.y = color_g;
		MenuState.RgbColor.z = color_b;
	...
	if (MenuState.GradientMenuTheme) {
		...
		float r1 = MenuState.MenuGradientColor1.x * 255.0f;
		float g1 = MenuState.MenuGradientColor1.y * 255.0f;
		float b1 = MenuState.MenuGradientColor1.z * 255.0f;

		float r2 = MenuState.MenuGradientColor2.x * 255.0f;
		float g2 = MenuState.MenuGradientColor2.y * 255.0f;
		float b2 = MenuState.MenuGradientColor2.z * 255.0f;
		...
		// Keep original alpha
		MenuState.MenuGradientColor = ImVec4(r, g, b, MenuState.MenuThemeColor.w);
	}
}
