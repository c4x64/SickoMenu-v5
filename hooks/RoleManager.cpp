...
void dRoleManager_SelectRoles(RoleManager* __this, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dRoleManager_SelectRoles executed");
	if (MenuState.DisableRoleManager) return RoleManager_SelectRoles(__this, method);
...
{
	preChosenImpCount = 0;
	if (MenuState.BattleRoyale || MenuState.TaskSpeedrun) return;
	for (size_t i = 0; i < MenuState.assignedRolesPlayer.size(); i++)
	{
		auto role = MenuState.assignedRoles[i];
		auto player = MenuState.assignedRolesPlayer[i];
		...
		if (MenuState.TournamentMode) {
			...
			if (std::find(MenuState.tournamentFriendCodes.begin(), MenuState.tournamentFriendCodes.end(), friendCode) == MenuState.tournamentFriendCodes.end()) {
				MenuState.tournamentFriendCodes.push_back(friendCode);
				MenuState.tournamentPoints[friendCode] = 0.f;
				MenuState.tournamentWinPoints[friendCode] = 0.f;
				MenuState.tournamentCalloutPoints[friendCode] = 0.f;
				MenuState.tournamentEarlyDeathPoints[friendCode] = 0.f;
				MenuState.tournamentKillCaps[friendCode] = 0.f;
			}
			if (trueRole == RoleTypes__Enum::Impostor || trueRole == RoleTypes__Enum::Shapeshifter || trueRole == RoleTypes__Enum::Phantom) {
				MenuState.tournamentAssignedImpostors.push_back(friendCode);
				MenuState.tournamentAliveImpostors.push_back(friendCode);
			}
		}
...
void AssignRoles(RoleRates& roleRates, int roleChance, RoleTypes__Enum role, il2cpp::List<List_1_PlayerControl_>& allPlayers, std::vector<uint8_t>& assignedPlayers)
{
	if (MenuState.BattleRoyale) {
	...
	else if (MenuState.TaskSpeedrun) {
		for (auto sender : GetAllPlayerControl()) {
			for (auto receiver : GetAllPlayerControl()) {
				auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), sender->fields._.NetId,
					uint8_t(RpcCalls__Enum::SetRole), SendOption__Enum::None, receiver->fields._.OwnerId, NULL);
		...
		//	if (MenuState.shapeshifters_amount + MenuState.impostors_amount >= maxImpostorAmount)
