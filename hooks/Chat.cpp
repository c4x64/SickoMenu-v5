...
void SendPrivateWarnMessage(PlayerControl* toPlayer, const std::string& reason, int totalWarns) {
	if (!MenuState.NotifyWarned || !toPlayer) return;

	if (!*Game::pLocalPlayer) return;

	if (IsHost() && MenuState.ChatCooldown >= 3.f) /* <- In order not to look ridiculous where we are not host :sob: */ {
...
void ChangeChatNotificationBackground(ChatNotification* chatNotif, PlayerControl* sender) {
	if ((!MenuState.PanicMode || MenuState.TempPanicMode) && chatNotif != NULL) {
		if (MenuState.CustomGameTheme && sender != NULL) {
			auto bg32 = Color32();
			bg32.r = int(MenuState.GameBgColor.x * 255); bg32.g = int(MenuState.GameBgColor.y * 255); bg32.b = int(MenuState.GameBgColor.z * 255); bg32.a = 255;
			auto bg = Color32_op_Implicit_1(bg32, NULL);
			auto text32 = Color32();
			text32.r = int(MenuState.GameTextColor.x * 255); text32.g = int(MenuState.GameTextColor.y * 255); text32.b = int(MenuState.GameTextColor.z * 255); text32.a = 255;
		...
		else if (MenuState.DarkMode && sender != NULL) {
...
void dChatController_AddChat(ChatController* __this, PlayerControl* sourcePlayer, String* chatText, bool censor, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dChatController_AddChat executed");
	...
	if (!MenuState.PanicMode) {
		...
		if (MenuState.CustomName && !MenuState.ServerSideCustomName && (sourcePlayer == *Game::pLocalPlayer || MenuState.CustomNameForEveryone)) {
		...
		if (MenuState.ReadGhostMessages) {
			bool wasDead = false;
		...
		if (MenuState.Enable_SMAC) {
			...
			if (MenuState.SMAC_CheckBadWords) {
				...
				for (auto word : MenuState.SMAC_BadWords) {
...
void dChatController_SetVisible(ChatController* __this, bool visible, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dChatController_SetVisible executed");
	if (MenuState.ChatAlwaysActive && !MenuState.PanicMode)
		ChatController_SetVisible(__this, true, method);
	else
	{
		MenuState.ChatActiveOriginalState = visible;
...
void dChatBubble_SetName(ChatBubble* __this, String* playerName, bool isDead, bool voted, Color color, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dChatBubble_SetName executed");
	if (!MenuState.PanicMode && (IsInGame() || IsInLobby())) {
		for (auto playerData : GetAllPlayerData()) {
			auto outfit = GetPlayerOutfit(playerData);
			if (outfit == NULL) continue;
			if (playerName == NetworkedPlayerInfo_get_PlayerName(playerData, nullptr)) {
				auto localData = GetPlayerData(*Game::pLocalPlayer);
				color = MenuState.RevealRoles ? GetRoleColor(playerData->fields.Role) :
				...
				if (MenuState.CustomName && !MenuState.ServerSideCustomName && (playerData == GetPlayerData(*Game::pLocalPlayer) || MenuState.CustomNameForEveryone)) {
					playerName = convert_to_string(GetCustomName(convert_from_string(playerName)));
				}
				if (MenuState.RevealRoles && IsInGame()) {
					playerName = convert_to_string("<size=50%>" + GetRoleName(playerData->fields.Role, MenuState.AbbreviatedRoleNames) + "</size> " + convert_from_string(playerName));
				}

				if (MenuState.PlayerColoredDots) {
				...
				if (MenuState.IsProcessingSickoChat) {
...
void dChatController_Update(ChatController* __this, MethodInfo* method) {
	...
	if (pool->fields.poolSize == 127 && !MenuState.ExtendChatHistory) {
	...
	if (__this->fields.state != ChatControllerState__Enum::Closed) {
		...
		if (MenuState.ShowChatTimer)
			chatCooldownText = MenuState.ChatCooldown >= 3.f ? (length == 0 ? "" : "<#0f0>You can chat now!</color> ") : std::format("<#f00>Wait for {:.1f} seconds!</color> ", 3 - MenuState.ChatCooldown);
	...
	if ((!MenuState.PanicMode || MenuState.TempPanicMode) && (MenuState.CustomGameTheme || MenuState.DarkMode)) {
	...
	MenuState.MessageSound = (AudioClip*)__this->fields.messageSound;

	auto chatText = __this->fields.freeChatField->fields.textArea->fields.text;
	__this->fields.freeChatField->fields.textArea->fields.AllowPaste = MenuState.ChatPaste && !MenuState.PanicMode && length == 0;
	...
	if (MenuState.ChatPaste && isCtrl && (isCpressed || isXpressed) && convert_from_string(chatText) != "") {
	...
	if (MenuState.MessageSent && MenuState.SafeMode) {
		__this->fields.timeSinceLastMessage = 0.f;
		MenuState.MessageSent = false;
	}
	MenuState.ChatCooldown = __this->fields.timeSinceLastMessage;
	MenuState.ChatFocused = __this->fields.freeChatField->fields.textArea->fields.hasFocus;

	if (!(IsHost() || !MenuState.SafeMode)) MenuState.ChatSpamMode = 0;

	if (IsChatValid(MenuState.chatMessage)) {
		if (!MenuState.PanicMode && MenuState.SafeMode && MenuState.ChatSpam && (MenuState.ChatSpamMode == 0 || MenuState.ChatSpamMode == 2) && (IsInGame() || IsInLobby()) && __this->fields.timeSinceLastMessage >= 3.5f) {
			PlayerControl_RpcSendChat(*Game::pLocalPlayer, convert_to_string(MenuState.chatMessage), NULL);
			//remove rpc queue stuff cuz of delay and anticheat kick
			MenuState.MessageSent = true;
		}
		if (!MenuState.PanicMode && MenuState.SafeMode && MenuState.CrashChatSpam && (MenuState.ChatSpamMode == 0 || MenuState.ChatSpamMode == 2) && (IsInGame() || IsInLobby()) && __this->fields.timeSinceLastMessage >= 3.5f) {
			PlayerControl_RpcSendChat(*Game::pLocalPlayer, convert_to_string(MenuState.chatMessage), NULL);
			//remove rpc queue stuff cuz of delay and anticheat kick
			MenuState.MessageSent = true;
		}
	...
	if (!MenuState.PanicMode && MenuState.AprilFoolsMode && MenuState.BrainrotEveryone && (IsInGame() || IsInLobby()) && (__this->fields.timeSinceLastMessage >= 3.5f || !MenuState.SafeMode)) {
	...
	if (!MenuState.PanicMode && MenuState.AprilFoolsMode && MenuState.DiddyPartyMode && MenuState.RizzUpEveryone && (IsInGame() || IsInLobby()) && (__this->fields.timeSinceLastMessage >= 3.5f || !MenuState.SafeMode)) {
	...
	/*if ((!MenuState.PanicMode || MenuState.TempPanicMode) && (MenuState.DarkMode || MenuState.CustomGameTheme) && __this->fields.freeChatField != NULL) {
...
{
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dTextBoxTMP_IsCharAllowed executed");
	...
	if (!MenuState.PanicMode) return (unicode_char != 0x08 && unicode_char != 0x0D && unicode_char != 0x7F && ((MenuState.SafeMode && unicode_char != 0x3C && unicode_char != 0x3E && unicode_char != 0x5B) || !MenuState.SafeMode));
...
{
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dTextBoxTMP_SetText executed");
	if (__this->fields.ForceUppercase) {
		__this->fields.SendOnFullChars = MenuState.PanicMode || !MenuState.BetterLobbyCodeInput;
		__this->fields.ClearOnFocus = MenuState.PanicMode || !MenuState.BetterLobbyCodeInput;
	...
	if (!MenuState.PanicMode) {
		if (!MenuState.SafeMode)
			...
			__this->fields.characterLimit = MenuState.ExtendChatLimit ? 120 : 100;
	}
...
{
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dPlayerControl_RpcSendChat executed");
...
void dChatBubble_SetText(ChatBubble* __this, String* chatText, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dChatBubble_SetText executed");
...
void dChatController_SendFreeChat(ChatController* __this, MethodInfo* method) {
	...
	if (!MenuState.PanicMode) {
		auto playerToChatAs = (!MenuState.SafeMode && MenuState.activeChatSpoof && MenuState.playerToChatAs.has_value()) ? MenuState.playerToChatAs.validate().get_PlayerControl() : *Game::pLocalPlayer;
		if (MenuState.ReadAndSendSickoChat && chatTextLower.substr(0, 4) == "/sc ") {
			if (IsInGame()) MenuState.rpcQueue.push(new RpcForceSickoChat(PlayerSelection(playerToChatAs), chatText.substr(4), true));
			if (IsInLobby()) MenuState.lobbyRpcQueue.push(new RpcForceSickoChat(PlayerSelection(playerToChatAs), chatText.substr(4), true));
...
// Added as an extension to the "Whitelisted Players Only" feature and other commands
		if (MenuState.ExtraCommands) {
			...
			if (chatTextLower.substr(0, 6) == "/warn ") {
				...
				int& warnCount = MenuState.WarnedFriendCodes[fc];
				warnCount++;
				MenuState.WarnReasons[fc].push_back(warnReason);
				MenuState.Save();
			...
			if (chatTextLower.substr(0, 8) == "/unwarn ") {
				...
				auto it = MenuState.WarnReasons.find(fc);
				if (it != MenuState.WarnReasons.end() && reasonIndex >= 0 && reasonIndex < (int)it->second.size()) {
					it->second.erase(it->second.begin() + reasonIndex);

					if (--MenuState.WarnedFriendCodes[fc] <= 0) {
			...
			if (chatTextLower.substr(0, 12) == "/checkwarns ") {
				...
				if (it != MenuState.WarnReasons.end() && !it->second.empty()) {
			...
			if (chatTextLower == "/kick all") {
				bool isBan = false;
				if (IsInGame()) {
					MenuState.rpcQueue.push(new PunishEveryone(isBan));
				}
				else if (IsInLobby()) {
					MenuState.lobbyRpcQueue.push(new PunishEveryone(isBan));
				}
				return;
			}
		...
		if (MenuState.activeWhisper && MenuState.playerToWhisper.has_value()) {
			MessageWriter* writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient),
				...
				MenuState.playerToWhisper.get_PlayerControl().value_or(nullptr)->fields._.OwnerId, NULL);
			std::string whisperMsg = std::format("{} whispers to you:\n{}",
				...
				chatText);
			if (whisperMsg.length() <= 100 || !MenuState.SafeMode)
			...
			std::string whisperMsgSelf = std::format("You whisper to {}:\n{}",
				RemoveHtmlTags(convert_from_string(NetworkedPlayerInfo_get_PlayerName(MenuState.playerToWhisper.get_PlayerData().value_or(nullptr), NULL))),
				chatText);
		...
		else if (!MenuState.SafeMode && MenuState.activeChatSpoof && MenuState.playerToChatAs.has_value()) {
			auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), GetPlayerControlById(MenuState.playerToChatAs.get_PlayerId())->fields._.NetId,
			...
			InnerNetClient_FinishRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), writer, NULL);
			dChatController_AddChat(Game::HudManager.GetInstance()->fields.Chat, GetPlayerControlById(MenuState.playerToChatAs.get_PlayerId()), convert_to_string(chatText), false, NULL);
		}
		else {
...
void dChatNotification_SetUp(ChatNotification* __this, PlayerControl* sender, String* text, MethodInfo* method) {
	if (!MenuState.PanicMode && MenuState.BetterChatNotifications) return;
...
AudioSource* dSoundManager_PlaySound(SoundManager* __this, AudioClip* clip, bool loop, float volume, AudioMixerGroup* audioMixer, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dSoundManager_PlaySound executed");
	if (MenuState.BetterMessageSounds && MenuState.MessageSound != NULL && clip == MenuState.MessageSound) volume = 0.f;
