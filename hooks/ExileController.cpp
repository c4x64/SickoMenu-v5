...
void dExileController_ReEnableGameplay(ExileController* __this, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dExileController_ReEnableGameplay executed");
	...
	try {// ESP: Reset Kill Cooldown
		...
		if (MenuState.GodMode && ((IsHost() && IsInGame()) || !MenuState.SafeMode)) {
...
void dExileController_BeginForGameplay(ExileController* __this, NetworkedPlayerInfo* exiled, bool voteTie, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dExileController_BeginForGameplay executed");
	ExileController_BeginForGameplay(__this, exiled, voteTie, method);
	exiledInfo = exiled;
	try {
		if (IsHost() && MenuState.TournamentMode && !voteTie && exiled != NULL) {
			if (PlayerIsImpostor(exiled)) {
				UpdatePoints(exiled, -1); //ImpVoteOut
				for (auto i : MenuState.voteMonitor) {
				...
				auto pos = std::find(MenuState.tournamentAliveImpostors.begin(), MenuState.tournamentAliveImpostors.end(), exiledFc);
				if (pos != MenuState.tournamentAliveImpostors.end()) MenuState.tournamentAliveImpostors.erase(pos);
			}
			else {
				for (auto p : GetAllPlayerData()) {
					if (PlayerIsImpostor(p) && !p->fields.IsDead) {
						...
						if (MenuState.tournamentKillCaps[friendCode] < 3.f) {
							MenuState.tournamentKillCaps[friendCode] += 1.f;
