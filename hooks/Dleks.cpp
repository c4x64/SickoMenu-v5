...
void LogIfEnabled(const std::string& message) {
    if (MenuState.ShowHookLogs) {
        LOG_DEBUG(message.c_str());
    }
}

bool ShouldDisableHostAnticheat() {
	return MenuState.DisableHostAnticheat && MenuState.CurrentScene != "MatchMaking" && MenuState.CurrentScene != "FindAGame" && !MenuState.IsFreePlay;
}

int32_t dConstants_1_GetBroadcastVersion(MethodInfo* method) {
    LogIfEnabled("Hook dConstants_1_GetBroadcastVersion executed");
    if (MenuState.CurrentScene == "" || MenuState.CurrentScene == "SplashIntro" || MenuState.CurrentScene == "MatchMaking" || MenuState.CurrentScene == "Tutorial" ||
        (MenuState.CurrentScene == "MainMenu" && MenuState.ShouldIgnoreBroadcastVersionHook)) {
...
/*AsyncOperationHandle_1_UnityEngine_GameObject_ dAssetReference_InstantiateAsync_1(AssetReference* __this, Transform* parent, bool instantiateInWorldSpace, MethodInfo* method) {
    ...
    LOG_DEBUG(std::format("AssetReference_InstantiateAsync executed with scene {}", MenuState.CurrentScene).c_str());
...
bool dAprilFoolsMode_ShouldFlipSkeld(MethodInfo* method) {
    if (IsHost()) return MenuState.FlipSkeld;
    MenuState.FlipSkeld = AprilFoolsMode_ShouldFlipSkeld(method);
    return MenuState.FlipSkeld;
}
...
void dMainMenuManager_LateUpdate(MainMenuManager* __this, MethodInfo* method) {
    ...
    MenuState.ShouldIgnoreBroadcastVersionHook = GameObject_GetActive(__this->fields.gameModeButtons, NULL);
}
