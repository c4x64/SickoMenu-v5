...
float dVent_CanUse(Vent* __this, NetworkedPlayerInfo* pc, bool* canUse, bool* couldUse, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dVent_CanUse executed");
	if (!MenuState.PanicMode && (MenuState.UnlockVents || (*Game::pLocalPlayer)->fields.inVent)) {
...
void dVent_EnterVent(Vent* __this, PlayerControl* pc, MethodInfo * method) {
	...
	if (!MenuState.PanicMode) {
		...
		synchronized(Replay::replayEventMutex) {
			...
			MenuState.liveConsoleEvents.emplace_back(std::make_unique<VentEvent>(GetEventPlayerControl(pc).value(), ventVector2D, VENT_ACTIONS::VENT_ENTER));
		}
		if (MenuState.confuser && MenuState.confuseOnVent && pc == *Game::pLocalPlayer)
...
void* dVent_ExitVent(Vent* __this, PlayerControl* pc, MethodInfo* method) {
	...
	if (!MenuState.PanicMode) {
		...
		synchronized(Replay::replayEventMutex) {
			...
			MenuState.liveConsoleEvents.emplace_back(std::make_unique<VentEvent>(GetEventPlayerControl(pc).value(), ventVector2D, VENT_ACTIONS::VENT_EXIT));
		}
	}
...
bool dVent_TryMoveToVent(Vent* __this, Vent* otherVent, String** error, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dVent_TryMoveToVent executed");
	if (!MenuState.PanicMode && *Game::pLocalPlayer != NULL) {
		bool wasVisible = PlayerControl_get_Visible(*Game::pLocalPlayer, NULL) && !(*Game::pLocalPlayer)->fields.walkingToVent && MenuState.ShowPlayersInVents && !GetPlayerData(*Game::pLocalPlayer)->fields.IsDead;
		if (wasVisible) {
...
/*void dVentilationSystem_Update(VentilationSystem_Operation__Enum op, int32_t ventId, MethodInfo* method) {
	...
	if (MenuState.FlipSkeld && IsHost() && op == VentilationSystem_Operation__Enum::Exit && *Game::pLocalPlayer != NULL)
...
void dPlayerPhysics_RpcExitVent(PlayerPhysics* __this, int32_t id, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dPlayerPhysics_RpcExitVent executed");
	PlayerPhysics_RpcExitVent(__this, id, method);
	/*if (MenuState.FlipSkeld && IsHost() && *Game::pLocalPlayer != NULL && __this->fields.myPlayer != NULL && __this->fields.myPlayer == *Game::pLocalPlayer)
		(*Game::pLocalPlayer)->fields.inVent = false; // Fix venting on Dleks*/
