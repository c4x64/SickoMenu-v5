...
{
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dLobbyBehaviour_Start executed");
	MenuState.LobbyTimer = 600;
	LobbyBehaviour_Start(__this, method);
	if (IsHost()) MenuState.JoinedAsHost = true;
}

void dLobbyBehaviour_Update(LobbyBehaviour* __this, MethodInfo* method)
{
	static bool hasStarted = true;
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dLobbyBehaviour_Update executed");
	LobbyBehaviour_Update(__this, method);
	if (MenuState.DisableLobbyMusic) {
		hasStarted = false;
...
void dMatchMakerGameButton_SetGame(MatchMakerGameButton* __this, GameListing gameListing, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dMatchMakerGameButton_SetGame executed");
	/*if (MenuState.PanicMode || !MenuState.ShowLobbyInfo) return MatchMakerGameButton_SetGame(__this, gameListing, method);
...
void dGameContainer_SetupGameInfo(GameContainer* __this, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dGameContainer_SetupGameInfo executed");
	if (MenuState.PanicMode || !MenuState.ShowLobbyInfo) return GameContainer_SetupGameInfo(__this, method);
	...
	std::string ageCol = getHexCodeFromImVec4(MenuState.AgeColor);
	std::string daterCol = getHexCodeFromImVec4(MenuState.DaterNamesColor);
	std::string nameCheckerCol = getHexCodeFromImVec4(MenuState.NameCheckerColor);
	std::string lobbyCodeCol = getHexCodeFromImVec4(MenuState.LobbyCodeColor);
...
void dGameStartManager_Update(GameStartManager* __this, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dGameStartManager_Update executed");
	try {
		if (*Game::pLobbyBehaviour != NULL)
			MenuState.LobbyTimer -= Time_get_deltaTime(NULL);
		...
		if (LobbyCode != "LWQQQQ" && LobbyCode != "QQQQQQ") {
			int LobbyTime = (int)MenuState.LobbyTimer;
			std::string lobbyTimeDisplay = "";
			if (!MenuState.PanicMode && MenuState.ShowLobbyTimer && IsHost()) {
				if (LobbyTime < 0)
					lobbyTimeDisplay = std::format(" <#0f0>({}0:00)</color>", MenuState.JoinedAsHost ? "" : "~");
				else if (LobbyTime <= 60)
					lobbyTimeDisplay = std::format(" <#f00>({}{}:{}{})</color>", MenuState.JoinedAsHost ? "" : "~", int(LobbyTime / 60), LobbyTime % 60 < 10 ? "0" : "", LobbyTime % 60);
				else if (LobbyTime <= 180)
					lobbyTimeDisplay = std::format(" <#ff0>({}{}:{}{})</color>", MenuState.JoinedAsHost ? "" : "~", int(LobbyTime / 60), LobbyTime % 60 < 10 ? "0" : "", LobbyTime % 60);
				else
					lobbyTimeDisplay = std::format(" ({}{}:{}{})", MenuState.JoinedAsHost ? "" : "~", int(LobbyTime / 60), LobbyTime % 60 < 10 ? "0" : "", LobbyTime % 60);
			}
			/*std::string glitchDisplay = "";
			if (!MenuState.PanicMode && MenuState.ShowLobbyInfo) {
			...
			if (state.hideCode && IsStreamerMode() && !MenuState.PanicMode && LobbyCode != "") {
				...
				if (MenuState.RgbLobbyCode && !MenuState.PanicMode)
					TMP_Text_set_text((TMP_Text*)__this->fields.GameRoomNameCode, convert_to_string(MenuState.rgbCode + /*glitchDisplay +*/ LobbyCode + lobbyTimeDisplay), NULL);
				else
	...
	MenuState.IsStartCountdownActive = __this->fields.startState == GameStartManager_StartingStates__Enum::Countdown;
	if (IsHost() && MenuState.IsStartCountdownActive && MenuState.CancelingStartGame) {
		GameStartManager_ResetStartState(__this, NULL);
	}
	MenuState.CancelingStartGame = false;
	bool buttonCheck = __this->fields.LastPlayerCount >= __this->fields.MinPlayers;
	__this->fields.MinPlayers = !MenuState.PanicMode && MenuState.AlwaysAllowStart ? (std::min)(__this->fields.LastPlayerCount, 4) : 4;
...
void dGameStartManager_ReallyBegin(GameStartManager* __this, bool neverShow, MethodInfo* method) {
	...
	if (IsHost() && MenuState.ModifyStartCountdown) {
		__this->fields.countDownTimer = MenuState.StartCountdown + 0.0001f;
