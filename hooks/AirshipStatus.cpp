...
{
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dAirshipStatus_OnEnable executed");
	AirshipStatus_OnEnable(__this, method);
	MenuState.mapType = Settings::MapType::Airship;
	if (IsHost() && MenuState.TaskSpeedrun && MenuState.GameLoaded)
...
float dAirshipStatus_CalculateLightRadius(AirshipStatus* __this, NetworkedPlayerInfo* player, MethodInfo* method) {
	if (MenuState.ShowHookLogs) LOG_DEBUG("Hook dAirshipStatus_CalculateLightRadius executed");
