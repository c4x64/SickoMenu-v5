...
#define JSON_TRYGET(key, value) \
        try { \
            ...
            SickoLog.Info(e.what()); \
        }

        if (MenuState.selectedConfig != "") JSON_TRYGET("SelectedConfig", this->selectedConfig);
    }
    catch (...) {
        //SickoLog.Info("Unable to load sicko-selected-config.json");
    }
...
#define JSON_TRYGET(key, value) \
        ...
        SickoLog.Info("Unable to load " + std::format("sicko-config/{}.json", this->selectedConfig));
    }
...
void Settings::SaveConfig() {
    ...
    if (this->selectedConfig != "") {
        try {
            ...
            //SickoLog.Info("Unable to save sicko-selected-config.json");
        }
    }
}
...
void Settings::Save() {
    ...
    if (this->selectedConfig != "") {
        ...
        try {
            ...
            SickoLog.Info("Unable to save " + std::format("sicko-config/{}.json", this->selectedConfig));
        }
        ...
        {
            SickoLog.Info("Unable to save friends.json");
        }*/
    }
}
