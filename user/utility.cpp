...
RoleRates::RoleRates(const class GameOptions& gameOptions, int playerAmount) {
	...
	if (MenuState.CustomImpostorAmount)
...
int GetMaxImpostorAmount(int playerAmount)
{
	GameOptions options;
	if (MenuState.CustomImpostorAmount)
		return MenuState.ImpostorCount;
...
#define TutorialScene (!MenuState.CurrentScene.compare("Tutorial"))
...
bool IsModdedHost() {
	return MenuState.DisableHostAnticheat;
}

bool IsInGame() {
	if (Object_1_IsNull((Object_1*)*Game::pAmongUsClient)) return false;
	if (!app::GameManager_get_Instance(nullptr)) return false;
	return (LocalInGame || OnlineInGame || TutorialScene) && (Object_1_IsNotNull((Object_1*)*Game::pShipStatus) || MenuState.GameLoaded) && Object_1_IsNotNull((Object_1*)*Game::pLocalPlayer);
}

bool IsInMultiplayerGame() {
	if (Object_1_IsNull((Object_1*)*Game::pAmongUsClient)) return false;
	if (!app::GameManager_get_Instance(nullptr)) return false;
	return (LocalInGame || OnlineInGame) && (Object_1_IsNotNull((Object_1*)*Game::pShipStatus) || MenuState.GameLoaded) && Object_1_IsNotNull((Object_1*)*Game::pLocalPlayer);
}
...
bool IsChatValid(std::string msg) {
	if (!MenuState.SafeMode) return true;
	/*(std::vector<std::string> properChars = {}; //check properly for length
	String* blank = convert_to_string("");
	std::string last_char = "";
	for (size_t i = 0; i < str.length(); i++) {
...
il2cpp::List<List_1_PlayerControl_> GetAllPlayerControl(/*bool includeFriends*/) {
	...
	/*else {
		if (MenuState.InGameFriends.size() == 0) {
			return *Game::pAllPlayerControls;
		}
		...
		for (size_t i = 0; i < max; i++) {
			if (MenuState.InGameFriends.contains(ret[i]->fields.PlayerId)) {
				ret.erase(i);
			}
		}
		return ret;
...
void RepairSabotage(PlayerControl* player) {
	if (MenuState.mapType == Settings::MapType::Ship || MenuState.mapType == Settings::MapType::Hq || MenuState.mapType == Settings::MapType::Fungle)
		MenuState.rpcQueue.push(new RpcUpdateSystem(SystemTypes__Enum::Reactor, 16));
	else if (MenuState.mapType == Settings::MapType::Pb)
		MenuState.rpcQueue.push(new RpcUpdateSystem(SystemTypes__Enum::Laboratory, 16));
	else if (MenuState.mapType == Settings::MapType::Airship) {
		MenuState.rpcQueue.push(new RpcUpdateSystem(SystemTypes__Enum::HeliSabotage, 16));
		MenuState.rpcQueue.push(new RpcUpdateSystem(SystemTypes__Enum::HeliSabotage, 17));
	}

	if (MenuState.mapType == Settings::MapType::Ship || MenuState.mapType == Settings::MapType::Hq)
		MenuState.rpcQueue.push(new RpcUpdateSystem(SystemTypes__Enum::LifeSupp, 16));

	MenuState.rpcQueue.push(new RpcUpdateSystem(SystemTypes__Enum::Comms, 16));
	if (MenuState.mapType == Settings::MapType::Hq || MenuState.mapType == Settings::MapType::Fungle) MenuState.rpcQueue.push(new RpcUpdateSystem(SystemTypes__Enum::Comms, 17));
	/*else if ("MushroomMixupSabotageTask"sv == sabotageTask->klass->_0.name) {
		MenuState.rpcQueue.push(new RpcUpdateSystem(SystemTypes__Enum::MushroomMixupSabotage, 0));
...
void CompleteAllTasks(PlayerControl* player) {
	...
	if (MenuState.SafeMode && player != *Game::pLocalPlayer) return;
	auto playerTasks = GetNormalPlayerTasks(player);
	for (auto playerTask : playerTasks) {
		if (playerTask->fields.taskStep < playerTask->fields.MaxStep) {
			MenuState.taskRpcQueue.push(new RpcForceCompleteTask(player, playerTask->fields._._Id_k__BackingField));
		}
	}
}
...
SystemTypes__Enum GetSystemTypes(const Vector2& vector) {
	...
	return MenuState.mapType == Settings::MapType::Fungle ? SystemTypes__Enum::Beach : SystemTypes__Enum::Outside;
}
...
{
	return (MenuState.mapType == Settings::MapType::Airship);
}
...
void ImpersonateName(__maybenull NetworkedPlayerInfo* data)
{
	...
	if (IsHost() || !MenuState.SafeMode) {
		if (IsInGame())
			MenuState.rpcQueue.push(new RpcSetName(playerName + (std::string(" ") * size_t(fillers))));
		else if (IsInLobby())
			MenuState.lobbyRpcQueue.push(new RpcSetName(playerName + (std::string(" ") * size_t(fillers))));
	}
	else if (IsNameValid(playerName) && playerName.length() <= size_t(12 - fillers)) {
		if (IsInGame())
			MenuState.rpcQueue.push(new RpcSetName(playerName + " "));
		else if (IsInLobby())
			MenuState.lobbyRpcQueue.push(new RpcSetName(playerName + " "));
	}
	else {
		if (IsInGame())
			MenuState.rpcQueue.push(new RpcSetName(GenerateRandomString()));
		else if (IsInLobby())
			MenuState.lobbyRpcQueue.push(new RpcSetName(GenerateRandomString()));
	}
}
...
void ImpersonateOutfit(NetworkedPlayerInfo_PlayerOutfit* outfit)
{
	...
	if (IsInGame()) {
		MenuState.rpcQueue.push(new RpcSetColor((IsHost() || !MenuState.SafeMode) ? outfit->fields.ColorId : GetRandomColorId(), (IsHost() || !MenuState.SafeMode)));
		MenuState.rpcQueue.push(new RpcSetHat(outfit->fields.HatId));
		MenuState.rpcQueue.push(new RpcSetVisor(outfit->fields.VisorId));
		MenuState.rpcQueue.push(new RpcSetSkin(outfit->fields.SkinId));
		MenuState.rpcQueue.push(new RpcSetPet(outfit->fields.PetId));
		MenuState.rpcQueue.push(new RpcSetNamePlate(outfit->fields.NamePlateId));
	}
	else if (IsInLobby()) {
		MenuState.lobbyRpcQueue.push(new RpcSetColor((IsHost() || !MenuState.SafeMode) ? outfit->fields.ColorId : GetRandomColorId(), (IsHost() || !MenuState.SafeMode)));
		MenuState.lobbyRpcQueue.push(new RpcSetHat(outfit->fields.HatId));
		MenuState.lobbyRpcQueue.push(new RpcSetVisor(outfit->fields.VisorId));
		MenuState.lobbyRpcQueue.push(new RpcSetSkin(outfit->fields.SkinId));
		MenuState.lobbyRpcQueue.push(new RpcSetPet(outfit->fields.PetId));
		MenuState.lobbyRpcQueue.push(new RpcSetNamePlate(outfit->fields.NamePlateId));
	}
}
...
std::string GetGradientUsername(std::string str, ImVec4 color1, ImVec4 color2, int offset) {
	...
	std::string opener = "";
	if (MenuState.UnderlineName) opener += "<u>";
	if (MenuState.StrikethroughName) opener += "<s>";

	std::string closer = "";
	if (MenuState.UnderlineName) closer += "</s>";
	if (MenuState.StrikethroughName) closer += "</u>";
...
void RefreshChat(bool alsoShow) {
	...
	try {
		...
		GameObject_SetActive(chat->fields.chatButton, ((!MenuState.PanicMode && MenuState.ChatAlwaysActive) || MenuState.InMeeting || IsInLobby() || GetPlayerData(*Game::pLocalPlayer)->fields.IsDead), NULL);
		ChatController_SetVisible(chat, ((!MenuState.PanicMode && MenuState.ChatAlwaysActive) || MenuState.InMeeting || IsInLobby() || GetPlayerData(*Game::pLocalPlayer)->fields.IsDead), NULL);
...
void SaveOriginalAppearance()
{
	...
	MenuState.originalName = convert_from_string(NetworkedPlayerInfo_get_PlayerName(GetPlayerData(*Game::pLocalPlayer), nullptr));
	MenuState.originalSkin = outfit->fields.SkinId;
	MenuState.originalHat = outfit->fields.HatId;
	MenuState.originalPet = outfit->fields.PetId;
	MenuState.originalColor = outfit->fields.ColorId;
	MenuState.activeImpersonation = false;
	MenuState.originalVisor = outfit->fields.VisorId;
	MenuState.originalNamePlate = outfit->fields.NamePlateId;
}
...
void ResetOriginalAppearance()
{
	try {
		...
		MenuState.originalSkin = originalSkin;
		MenuState.originalHat = originalHat;
		MenuState.originalPet = originalPet;
		MenuState.originalColor = MenuState.SelectedColorId;
		MenuState.originalVisor = originalVisor;
		MenuState.originalNamePlate = originalNamePlate;
	}
	catch (...) {
...
void ControlAppearance(bool randomize)
{
	try {
		...
		if (randomize) {
			...
			if (MenuState.confuserNameGeneration == 0 || (MenuState.cyclerNameGeneration == 2 && ((IsHost() || !MenuState.SafeMode) ? MenuState.cyclerUserNames.empty() : validNames.empty())))
				name = GenerateRandomString();
			else if (MenuState.confuserNameGeneration == 1)
				name = GenerateRandomString(true);
			else if (MenuState.confuserNameGeneration == 2) {
				if ((IsHost() || !MenuState.SafeMode) && !MenuState.cyclerUserNames.empty())
					name = MenuState.cyclerUserNames[randi(0, MenuState.cyclerUserNames.size() - 1)];
				else
			...
			if (IsHost() || !MenuState.SafeMode)
				queue->push(new RpcForceColor(*Game::pLocalPlayer, MenuState.originalColor));
			else
				queue->push(new RpcSetColor(MenuState.originalColor));
			queue->push(new RpcSetPet(MenuState.originalPet));
...
Color GetRoleColor(RoleBehaviour* roleBehaviour, bool gui) {
	if (roleBehaviour == nullptr)
		return MenuState.LightMode && gui ? Palette__TypeInfo->static_fields->Black : Palette__TypeInfo->static_fields->White;

	app::Color c;
	switch (roleBehaviour->fields.Role) {
	case RoleTypes__Enum::CrewmateGhost: {
		c = GetColorFromImVec4(MenuState.CrewmateGhostColor);
		break;
	}
	case RoleTypes__Enum::Crewmate: {
		c = GetColorFromImVec4(MenuState.CrewmateColor);
		break;
	}
	case RoleTypes__Enum::Engineer: {
		c = GetColorFromImVec4(MenuState.EngineerColor);
		break;
	}
	case RoleTypes__Enum::GuardianAngel: {
		c = GetColorFromImVec4(MenuState.GuardianAngelColor);
		break;
	}
	case RoleTypes__Enum::Scientist: {
		c = GetColorFromImVec4(MenuState.ScientistColor);
		break;
	}
	case RoleTypes__Enum::Impostor: {
		c = GetColorFromImVec4(MenuState.ImpostorColor);
		break;
	}
	case RoleTypes__Enum::Shapeshifter: {
		c = GetColorFromImVec4(MenuState.ShapeshifterColor);
		break;
	}
	case RoleTypes__Enum::ImpostorGhost: {
		c = GetColorFromImVec4(MenuState.ImpostorGhostColor);
		break;
	}
	case RoleTypes__Enum::Noisemaker: {
		c = GetColorFromImVec4(MenuState.NoisemakerColor);
		break;
	}
	case RoleTypes__Enum::Tracker: {
		c = GetColorFromImVec4(MenuState.TrackerColor);
		break;
	}
	case RoleTypes__Enum::Phantom: {
		c = GetColorFromImVec4(MenuState.PhantomColor);
		break;
	}
	default: {
		c = GetColorFromImVec4(MenuState.CrewmateColor);;
...
{
	return (MenuState.FlipSkeld && GameOptions().GetByte(app::ByteOptionNames__Enum::MapId) == 3) ? x - 50.0f : x;
}
...
std::string GetCustomName(std::string name, bool forceUnique, uint8_t id, int offset) {
	...
	if (MenuState.UsePrefixAndSuffix) {
		if (MenuState.NamePrefix != "") {
			auto prefix = RemoveHtmlTags(MenuState.NamePrefix);
			if (name.starts_with(prefix)) name = name.substr(prefix.size());
		}
		if (MenuState.NameSuffix != "") {
			auto suffix = RemoveHtmlTags(MenuState.NameSuffix);
			if (name.ends_with(suffix)) name = name.substr(0, name.size() - suffix.size());
		}
	}

	std::string opener = "", closer = "";
	if (MenuState.RgbName) {
		if (MenuState.RgbMethod == 0) {
			opener = MenuState.rgbCode;
			closer = "</color>";
		}
		else {
			std::string newName = "";
			...
			for (size_t i = 0; i < name.length(); i++) {
				...
				const auto calculate = [](float value) {return sinf(value) * .5f + .5f; };
	...
	if (MenuState.ColoredName && !MenuState.RgbName) {
		name = GetGradientUsername(name, MenuState.NameColor1, MenuState.NameColor2, MenuState.ColorMethod == 0 ? 0 : offset);
	}

	if (MenuState.ItalicName) {
		opener += "<i>";
		closer += "</i>";
	}

	if (MenuState.UnderlineName && (!MenuState.ColoredName || MenuState.RgbName)) {
		opener += "<u>";
		closer += "</u>";
	}

	if (MenuState.StrikethroughName && (!MenuState.ColoredName || MenuState.RgbName)) {
		opener += "<s>";
		closer += "</s>";
	}

	if (MenuState.BoldName) {
		opener += "<b>";
		closer += "</b>";
	}

	if (MenuState.NobrName) {
		opener += "<nobr>";
		closer += "</nobr>";
	}

	if (MenuState.Font) {
		switch (MenuState.FontType) {
		case 0: {
	...
	if (MenuState.ResizeName) {
		opener += std::format("<size={}%>", MenuState.NameSize * 100);
		closer += "</size>";
	}

	if (MenuState.IndentName) {
		opener += std::format("<line-indent={}>", MenuState.NameIndent * 1);
		closer += "</line-indent>";
	}

	if (MenuState.CspaceName) {
		opener += std::format("<cspace={}>", MenuState.NameCspace * 1);
		closer += "</cspace>";
	}

	if (MenuState.MspaceName) {
		opener += std::format("<mspace={}>", MenuState.NameMspace * 1);
		closer += "</mspace>";
	}

	if (MenuState.VoffsetName) {
		opener += std::format("<voffset={}>", MenuState.NameVoffset * 1);
		closer += "</voffset>";
	}

	if (MenuState.RotateName) {
		opener += std::format("<rotate={}>", MenuState.NameRotate * 1);
		closer += "<rotate=0>";
	}
	if (forceUnique) opener = std::format("<size=0><{}></size>", id) + opener;

	if (MenuState.UsePrefixAndSuffix) {
		if (MenuState.NamePrefix != "") opener = MenuState.NamePrefix + (MenuState.PrefixAndSuffixNewLines ? "<br>" : "") + opener;
		if (MenuState.NameSuffix != "") closer = closer + (MenuState.PrefixAndSuffixNewLines ? "<br>" : "") + MenuState.NameSuffix;
	}
...
bool CheckConfigExists(std::string configName) {
	auto path = getModulePath(hModule);
	auto settingsPath = path.parent_path() / std::format("sicko-config/{}.json", MenuState.selectedConfig);
	return std::filesystem::exists(settingsPath);
}

void UpdatePoints(NetworkedPlayerInfo* playerData, float points) {
	if (!MenuState.TournamentMode) return;
	std::string friendCode = convert_from_string(playerData->fields.FriendCode);
	MenuState.tournamentPoints[friendCode] += points;
}

void SMAC_OnCheatDetected(PlayerControl* pCtrl, std::string reason) {
	if (!MenuState.Enable_SMAC) return;
	if (reason == "Overloading" && !(IsHost() && MenuState.SMAC_HostPunishment >= 2)) return; // Don't spam logs for overloading, that causes overload as well
	...
	auto it = std::find(MenuState.WhitelistFriendCodes.begin(), MenuState.WhitelistFriendCodes.end(), fc);
	if (fc != "" && MenuState.SMAC_IgnoreWhitelist && it != MenuState.WhitelistFriendCodes.end()) return;
	if (fc != "" && MenuState.SMAC_AddToBlacklist) {
		if (it != MenuState.WhitelistFriendCodes.end()) MenuState.WhitelistFriendCodes.erase(it);
		MenuState.BlacklistFriendCodes.push_back(fc);
		MenuState.Save();
	}

	if (IsHost()) {
		switch (MenuState.SMAC_HostPunishment) {
		case 0:
		...
		case 1: {
			...
			std::string colorText = MenuState.CustomGameTheme ? std::format("<#{:02x}{:02x}{:02x}>",
				int(MenuState.GameTextColor.x * 255), int(MenuState.GameTextColor.y * 255), int(MenuState.GameTextColor.z * 255)) :
				MenuState.DarkMode ? "<#fff>" : "<#000>";
		...
		switch (MenuState.SMAC_Punishment) {
		case 0:
			LOG_INFO((name + " has been detected by SickoMenu Anticheat! Reason: " + reason).c_str());
			break;
		case 1: {
...
void SaveGameOptions(const class GameOptions& gameOptions) {
	/*MenuState.PlayerSpeed = MenuState.PrevPlayerSpeed = gameOptions.GetPlayerSpeedMod();
	MenuState.GACooldown = MenuState.PrevGACooldown = gameOptions.GetGACooldown();
	MenuState.KillDistance = MenuState.PrevKillDistance = gameOptions.GetInt(app::Int32OptionNames__Enum::KillDistance);
	MenuState.TaskBarUpdates = MenuState.PrevTaskBarUpdates = gameOptions.GetInt(app::Int32OptionNames__Enum::TaskBarMode);
	MenuState.VisualTasks = MenuState.PrevVisualTasks = gameOptions.GetBool(app::BoolOptionNames__Enum::VisualTasks);*/
	MenuState.mapHostChoice = gameOptions.GetMapId();
	MenuState.impostors_amount = gameOptions.GetNumImpostors();
}
...
void TrackPlayers()
{
	...
	auto& hist = MenuState.PlayerHistory;
	bool needSave = false;
	...
	for (auto p : GetAllPlayerControl())
	{
		...
		if (MenuState.RemovedPlayers.count(fc)) continue;
...
void AddToWhitelist(const std::string& fc)
{
	...
	{
		MenuState.WhitelistFriendCodes.push_back(fc);
		MenuState.Save();
	}
}
...
{
	if (std::find(MenuState.BlacklistFriendCodes.begin(), MenuState.BlacklistFriendCodes.end(), fc) == MenuState.BlacklistFriendCodes.end())
	{
		MenuState.BlacklistFriendCodes.push_back(fc);
		MenuState.Save();
	}
...
{
	auto it = std::find(MenuState.WhitelistFriendCodes.begin(), MenuState.WhitelistFriendCodes.end(), fc);
	if (it != MenuState.WhitelistFriendCodes.end())
	{
		MenuState.WhitelistFriendCodes.erase(it);
		MenuState.Save();
	}
}
...
{
	auto it = std::find(MenuState.BlacklistFriendCodes.begin(), MenuState.BlacklistFriendCodes.end(), fc);
	if (it != MenuState.BlacklistFriendCodes.end())
	{
		MenuState.BlacklistFriendCodes.erase(it);
		MenuState.Save();
	}
}
...
void GeneratePlatformId() {
	...
	{
		MenuState.FakeXboxId = GenerateXboxId();
		MenuState.SpoofXboxId = true;
		MenuState.Save();
	}

	if (MenuState.FakePlatform == 9)
	{
		MenuState.FakePsnId = GeneratePsnId();
		MenuState.SpoofPsnId = true;
		MenuState.Save();
	}
}
