...
bool GameVersionCheck() {
	...
	/*if (!IsWindows10OrGreater()) {
		SickoLog.Error("Version of windows not supported exiting!");
	...
	std::string gameAssemblyCRC = GetCRC32(gameAssembly); //We won't use this, but it will SickoLog it
...
void Run(LPVOID lpParam) {
	...
	{
		std::ostringstream ss;
		ss << "\n\tSickoMenu - " << __DATE__ << " - " << __TIME__ << std::endl; // SickoLog SickoMenu info
		/*ss << "\tBuild: " << _CONFIGURATION_NAME << std::endl;
		ss << "\tCommit: " << GetGitCommit() << " - " << GetGitBranch() << std::endl; // SickoLog git info*/
		ss << "\tVersion: " << MenuState.SickoVersion << std::endl;
		ss << "\tAmong Us Version: " << getGameVersion() << std::endl; // SickoLog among us info
		LOG_INFO(ss.str());
#if _DEBUG
		SetConsoleTitleA(std::format("Debug Console - SickoMenu {} (Among Us v{})", MenuState.SickoVersion, getGameVersion()).c_str());
#endif
	}
#if _DEBUG
