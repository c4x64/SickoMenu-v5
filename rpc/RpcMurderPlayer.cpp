...
void RpcMurderLoop::Process()
{
	if (onlyOnTarget && (IsInMultiplayerGame() || IsInLobby())) {
	...
	else if ((IsInLobby() && MenuState.KillInLobbies) && target == *Game::pLocalPlayer) {
		for (size_t i = 1; i <= (size_t)count; ++i) {
			if (!PlayerSelection(Player).has_value() || !PlayerSelection(target).has_value()) break;
			for (auto p : GetAllPlayerControl()) {
				if (p != *Game::pLocalPlayer) {
		...
		if (IsInLobby() && !MenuState.KillInLobbies) return;
		for (size_t i = 1; i <= (size_t)count; ++i) {
			if (!PlayerSelection(Player).has_value() || !PlayerSelection(target).has_value()) break;
...
{
	if (!PlayerSelection(Player).has_value()) return;
	if (MenuState.ChatCooldown < 3.f) return;
...
void PunishEveryone::Process() {
	...
	for (auto playerControl : allPlayers) {
		...
		if (MenuState.Ban_IgnoreWhitelist &&
			std::find(MenuState.WhitelistFriendCodes.begin(), MenuState.WhitelistFriendCodes.end(),
				convert_from_string(playerData->fields.FriendCode)) != MenuState.WhitelistFriendCodes.end()) {
			continue;
		}
